...
struct MemSnapshot {
    ...
    int64_t m_meta_formula;  // total_flush_count * b   (sequential: 1 dev/flush)
...
class MemConstrainedWriter {
   ...
   private:
    ...
    void open_new_file() {
        std::string path = base_path_ + "_" + std::to_string(file_count_) + ".tsfile";

        write_file_ = new WriteFile();
        int ret = write_file_->create(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
        if (ret != 0) {
    ...
    void take_snapshot(const std::string& event) {
        ...
        // Direct
        s.m_data_direct = writer_ ? writer_->calculate_mem_size_for_all_group() : 0;
...
int main(int argc, char* argv[]) {
    ...
    auto schema = std::make_shared<TableSchema>(
        ...
        std::vector<ColumnSchema>{
            ColumnSchema("id1", STRING, UNCOMPRESSED, PLAIN, ColumnCategory::TAG),
            ColumnSchema("id2", STRING, UNCOMPRESSED, PLAIN, ColumnCategory::TAG),
    ...
    std::cout << "=== MemConstrainedWriter Experiment ===\n"
              ...
              << "  data_budget:        " << writer.data_budget()     / (1024*1024) << " MB\n"
              << "  meta_budget:        " << writer.meta_budget()     / (1024*1024) << " MB\n"
    ...
    for (int dev = 0; dev < kNumDevices; dev++) {
        ...
        for (int64_t off = 0; off < rows_per_dev; ) {
            ...
            for (uint32_t i = 0; i < n; i++) {
                ...
                tablet.add_value(i, "s1", static_cast<int32_t>(ts % 100000) + ni(rng));
                tablet.add_value(i, "s2", static_cast<int32_t>(ts % 50000)  + ni(rng));
                tablet.add_value(i, "s3", ts + static_cast<int64_t>(ni(rng)));
                tablet.add_value(i, "s4", ts * 2 + static_cast<int64_t>(ni(rng)));
                tablet.add_value(i, "s5", static_cast<float>(ts % 10000)  + nf(rng));
                tablet.add_value(i, "s6", static_cast<float>(ts % 5000)   + nf(rng));
