...
// Module IDs we care about for CSV output (the big ones).
struct ModEntry {
    common::AllocModID id;
    const char* name;
};
static const ModEntry kMods[] = {
    {common::MOD_DEFAULT, "DEFAULT"},
    {common::MOD_TVLIST_DATA, "TVLIST_DATA"},
    {common::MOD_TSBLOCK, "TSBLOCK"},
    {common::MOD_PAGE_WRITER_OUTPUT_STREAM, "PAGE_WRITER_OUTPUT_STREAM"},
    {common::MOD_CW_PAGES_DATA, "CW_PAGES_DATA"},
    {common::MOD_STATISTIC_OBJ, "STATISTIC_OBJ"},
    {common::MOD_ENCODER_OBJ, "ENCODER_OBJ"},
    {common::MOD_DECODER_OBJ, "DECODER_OBJ"},
    {common::MOD_TSFILE_WRITER_META, "TSFILE_WRITER_META"},
    {common::MOD_TSFILE_WRITE_STREAM, "TSFILE_WRITE_STREAM"},
    {common::MOD_COMPRESSOR_OBJ, "COMPRESSOR_OBJ"},
    {common::MOD_TS2DIFF_OBJ, "TS2DIFF_OBJ"},
    {common::MOD_TABLET, "TABLET"},
};
static const int kModCount = sizeof(kMods) / sizeof(kMods[0]);
...
static void write_csv_header(std::ofstream& csv) {
    ...
    for (int i = 0; i < kModCount; i++) csv << "," << kMods[i].name;
    csv << ",TOTAL\n";
}
...
static void write_csv_row(std::ofstream& csv, int64_t rows,
    ...
    for (int i = 0; i < kModCount; i++) {
        int32_t val = ms.get_stat(kMods[i].id);
        csv << "," << val;
        total += val;
    }
    csv << "," << total << "\n";
...
int main(int argc, char* argv[]) {
    ...
    // --- Create writer ---
    const std::string path = "write_memory_bench.tsfile";
    ...
    auto* schema = new storage::TableSchema(
        std::string(kTable),
        {
            ...
            common::ColumnSchema("s4", common::INT32,
                                 common::SNAPPY, common::PLAIN,
                                 common::ColumnCategory::FIELD),
            common::ColumnSchema("s5", common::INT64,
                                 common::SNAPPY, common::PLAIN,
                                 common::ColumnCategory::FIELD),
            common::ColumnSchema("s6", common::INT32,
                                 common::SNAPPY, common::PLAIN,
                                 common::ColumnCategory::FIELD),
        });
    ...
    auto fill_row = [](storage::Tablet& tablet, uint32_t row,
        ...
        tablet.add_value(row, "s1", ts);
        tablet.add_value(row, "s2", ts * 1.1);
        tablet.add_value(row, "s3", static_cast<float>(ts % 10000));
        tablet.add_value(row, "s4", static_cast<int32_t>(ts % 100000));
        tablet.add_value(row, "s5", ts * 2);
        tablet.add_value(row, "s6", static_cast<int32_t>(ts % 50000));
    };
