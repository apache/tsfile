...
TEST_F(TsFileWriterTableTest, EncodingConfigIntegration) {
    ...
    auto table_schema = new TableSchema("configTestTable", measurement_schemas, column_categories);
    ...
    reader.open(write_file_.get_file_path());
    ResultSet* ret = nullptr;
    int ret_value = reader.query("configTestTable", measurement_names, 0, 10, ret);
    ASSERT_EQ(common::E_OK, ret_value);
