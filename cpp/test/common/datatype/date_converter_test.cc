...
protected:
    void SetUp() override {
        ...
        valid_tm_ = {0, 0, 12, 3, 6, 125}; // tm_mday=3, tm_mon=6 (July), tm_year=125 (2025)
        valid_int_ = 20250703;
    }
...
TEST_F(DateConverterTest, BoundaryConditions) {
    ...
    EXPECT_EQ(DateConverter::date_to_int(invalid_leap, leap_int), common::E_INVALID_ARG);
...
TEST_F(DateConverterTest, InvalidInputs) {
    ...
    int32_t out_int;
    EXPECT_EQ(DateConverter::date_to_int(invalid_tm, out_int), common::E_INVALID_ARG);

    // Year out of range
    std::tm year_out_of_range = {0, 0, 12, 3, 6, -901}; // 0999-07-03 (year < 1000)
    EXPECT_EQ(DateConverter::date_to_int(year_out_of_range, out_int), common::E_INVALID_ARG);

    // Invalid integer format
    std::tm tm_result = {0};
    EXPECT_EQ(DateConverter::int_to_date(20251301, tm_result), common::E_INVALID_ARG); // month=13
    EXPECT_EQ(DateConverter::int_to_date(20250015, tm_result), common::E_INVALID_ARG); // month=0
}

// Test uninitialized fields
TEST_F(DateConverterTest, UninitializedFields) {
    std::tm uninitialized = {0}; // tm_year etc. are 0 (not explicitly initialized)
    uninitialized.tm_year = -1;  // Mark as invalid
    int32_t out_int;
    EXPECT_EQ(DateConverter::date_to_int(uninitialized, out_int), common::E_INVALID_ARG);
}
