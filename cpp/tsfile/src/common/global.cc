...
const char* s_data_type_names[7] = {
  "BOOLEAN",
  "INT32",
  "INT64",
  "FLOAT",
...
void cols_to_json(ByteStream *byte_stream, std::vector<common::ColumnDesc> &ret_ts_list)
{
  // 1. append start tag
  ...
  for (size_t i = 0; i < ts_count; ++i) {
    ...
    const char* compression = get_compression_name(ret_ts_list[i].compression_);
