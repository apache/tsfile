...
struct Value
{
  ...
  FORCE_INLINE bool is_type(TSDataType type) const
  {
    return type == type_;
  }
  ...
  {
    return is_type(BOOLEAN) || is_type(DOUBLE) ||
           is_type(TEXT) || is_type(INT64) || is_type(NULL_TYPE);
  }
  ...
  FORCE_INLINE void set_value(TSDataType type, T val)
  {
    ...
    switch (type) {
      ...
      default: {
        common::log_error("unknown data type");
      }
    }
  }
...
template<typename T>
FORCE_INLINE int get_typed_data_from_value(Value *value, T &ret_data) { return E_OK; }

template<> FORCE_INLINE int get_typed_data_from_value<bool>(Value *value, bool &ret_data)
{
  ret_data = value->value_.bval_;
  return (value->type_ == BOOLEAN) ? E_OK : E_TYPE_NOT_MATCH;
}

template<> FORCE_INLINE int get_typed_data_from_value<int32_t>(Value *value, int32_t &ret_data)
{
...
template<> FORCE_INLINE int get_typed_data_from_value<int64_t>(Value *value, int64_t &ret_data)
{
  ret_data = value->value_.lval_;
  return (value->type_ == INT64) ? E_OK : E_TYPE_NOT_MATCH;
}

template<> FORCE_INLINE int get_typed_data_from_value<float>(Value *value, float &ret_data)
{
...
template<> FORCE_INLINE int get_typed_data_from_value<double>(Value *value, double &ret_data)
{
  ret_data = value->value_.dval_;
  return (value->type_ == DOUBLE) ? E_OK : E_TYPE_NOT_MATCH;
}

// template<> FORCE_INLINE int get_typed_data_from_value<Text>(Value *value, Text &ret_data)
// {
