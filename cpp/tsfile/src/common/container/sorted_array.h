...
public:
...
#ifndef NDEBUG
  friend std::ostream& operator << (std::ostream &out, SortedArray<ValueType> &sa)
  {
  ...
  //   if (init() == E_OK) {
  //     memcpy(this->array_, other.array_, this->capacity_ * sizeof(*(this->array_))); 
  //     return E_OK;
  //   }
  //   return E_OOM;
  // }
  ...
  int init()
  {
    ...
    array_ = (ValueType *)mem_alloc(capacity_ * sizeof(*(array_)), MOD_ARRAY);
...
private:
  ...
  int extend()
  {
    ...
    array_ = (ValueType *)mem_realloc(array_, new_capacity * sizeof(*(array_)));
    if (UNLIKELY(nullptr == array_)) {
  ...
  int shrink()
  {
    ...
    // if the size passed to realloc is smaller than before, OS will automatically release the rest memory
    array_ = (ValueType *)mem_realloc(array_, new_capacity * sizeof(*(array_)));  // TODO: user ourself's mem_alloc()
    if (UNLIKELY(nullptr == array_)) {
      log_error("malloc failed.");
      return E_OOM;  
    }    
...
public:
  int insert(const ValueType &value)
  {
    ...
    memmove(array_ + (pos + 1), array_ + pos, (size_ - pos) * sizeof(*(array_)));  
    array_[pos] = value;
    size_++;
    return E_OK;
  }
  ...
  {
    if (idx >= size_) {
      log_error("index %lu is out of range, because size is %lu.", idx, size_);
      ASSERT(idx < size_);
      return dummy_;
    }
    return array_[idx];
  ...
  /*
   * if the value is not found, then set 'found' is false ant return the first bigger one's location
   */
  ...
  ValueType remove(size_t idx)
  {
    ...
    if (capacity_ > SORTED_ARRAY_SHRINK_THRESHOLD && size_ <= capacity_ / 4) {
      shrink(); 
    }
    ValueType ret = array_[idx];
    memmove(array_ + idx, array_ + idx + 1, (size_ - idx - 1) * sizeof(*(array_)));  
    size_--;
