...
enum ConfigLevel
{
  ...
  RESTART,   // Can be changed, but the database must be restarted to take effect
  USERSET    // Session level update
};

typedef struct ConfigValue
{
  uint32_t tsblock_mem_inc_step_size_;    // tsblock memory self-increment step size
...
// In the future, configuration items need to be dynamically adjusted according to the level
extern void set_config_value();
extern void config_set_page_max_point_count(uint32_t page_max_ponint_count);
extern void config_set_max_degree_of_index_node(uint32_t max_degree_of_index_node);
