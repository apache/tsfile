...
FORCE_INLINE int32_t float_to_int(float f)
{
  ...
  my_fi.f_ = f;
  return my_fi.i_;  // By mimicking the java jdk's implementation, it's essentially the same as line 75.
}
...
private:
  struct Page
  {
    ...
    {
      buf_ = (uint8_t*)(this + 1);  // I think it should add 17, because the Page class is 9+8=17 bytes. No, adding one to a pointer is not adding a byte, but adding the length of the byte corresponding to the type pointed to by the pointer, and adding one here is actually adding 17 bytes. No problem.
    }
    Page(bool enable_atomic, uint8_t *wrapped_buf)
      : next_(nullptr, enable_atomic),
        buf_(wrapped_buf) {}
  };

public:
  ByteStream(uint32_t page_size,
             AllocModID mid,
  ...
  void wrap_from(const char *buf, int32_t buf_len)
  {
    ...
    page_size_ = buf_len;
    std::cout<<"page_size when wrap from" << page_size_ << std::endl;
  ...
  FORCE_INLINE bool is_wrapped() const { return head_.load() == &wrapped_page_; }
  ...
  void shallow_clone_from(ByteStream &other)
  {
    ...
    std::cout << "page size after clone" << this->page_size_ << std::endl;
  ...
  // write @buf with length @len into this bytestream
  int write_buf(const uint8_t *buf, const uint32_t len)
  {
    ...
    while (write_len < len) {
      ...
      uint32_t copy_len = remainder < (len - write_len) ? remainder : (len - write_len);
      memcpy(tail_.load()->buf_ + total_size_.load() % page_size_, buf + write_len, copy_len);
  ...
  // read @want_len bytes to @buf, @read_len indicates real len we read.
  ...
  int read_buf(uint8_t *buf, const uint32_t want_len, uint32_t &read_len)
  {
    ...
    uint32_t want_len_limited = partial_read ? (total_size_.load() - read_pos_) : want_len;
    read_len = 0;
    while (read_len < want_len_limited) {
      ...
      uint32_t copy_len = remainder < want_len_limited - read_len ? remainder : want_len_limited - read_len;
      memcpy(buf + read_len, read_page_->buf_ + (read_pos_ % page_size_), copy_len);
      read_len += copy_len;
      read_pos_ += copy_len;
    }
  ...
  FORCE_INLINE int read_buf(char *buf, const uint32_t want_len, uint32_t &read_len)
  {
    return read_buf((uint8_t*)buf, want_len, read_len);
  }
  FORCE_INLINE int read_buf(char *buf, const int32_t want_len, int32_t &read_len)
  {
    return read_buf((uint8_t*)buf, (uint32_t&)want_len, (uint32_t&)read_len);
  }
  ...
  Buffer acquire_buf()
  {
    ...
    b.buf_ = (char*)(tail_.load()->buf_ + (total_size_.load() % page_size_));
  ...
  void buffer_used(uint32_t used_bytes)
  {
    ASSERT(used_bytes >= 1);
    // would not span page
    ASSERT((total_size_.load() / page_size_) == ((total_size_.load() + used_bytes - 1) / page_size_));
    total_size_.atomic_aaf(used_bytes);
  }

  /* ================ Part 4: reading internal buffers ================ */
  /*
   * one-shot read iterator
  ...
  struct BufferIterator
  {
    ...
    Buffer get_next_buf()
    {
      ...
      if (cur_ != nullptr) {
        ...
        if (cur_ == end_ && host_.total_size_.load() % host_.page_size_ != 0) {
  ...
  BufferIterator init_buffer_iterator() const { return BufferIterator(*this); }

  /*
   * producer-consumer mode read iterator
   * Note:
        ...
        if (cur_ == host_end) {
          if (host_total_size % host_.page_size_ == 0) {
            if (read_offset_within_cur_page_ == host_.page_size_) { 
              ...
              b.buf_ = ((char*)(cur_->buf_)) + read_offset_within_cur_page_;
              b.len_ = host_.page_size_ - read_offset_within_cur_page_;
              read_offset_within_cur_page_ = host_.page_size_;
              total_end_offset_ += b.len_;
              return b;
            }
          } else {
            if (read_offset_within_cur_page_ == (host_total_size % host_.page_size_)) {
              return b;
            } else {
              b.buf_ = ((char*)(cur_->buf_)) + read_offset_within_cur_page_;
              b.len_ = (host_total_size % host_.page_size_) - read_offset_within_cur_page_;
              read_offset_within_cur_page_ = (host_total_size % host_.page_size_);
              total_end_offset_ += b.len_;
              return b;
            }
          }
...
private:
  ...
  {
    int ret = common::E_OK;
    if (UNLIKELY(tail_.load() == nullptr || total_size_.load() % page_size_ == 0)) {
      Page *p = nullptr;
...
private:
  ...
  Page* read_page_; // only one thread is allow to read this ByteStream
  OptionalAtomic<uint32_t> total_size_; // total size in byte
  uint32_t read_pos_; // current read position
  uint32_t marked_read_pos_; // current read position
  uint32_t page_size_;
  AllocModID mid_;
  Page wrapped_page_;
};

FORCE_INLINE int merge_byte_stream(ByteStream &sea, ByteStream &river, bool purge_river = false)
{
  int ret = common::E_OK;
...
FORCE_INLINE uint32_t get_var_uint_size(uint32_t ui32)  // return: the length of usigned number after varint encoding.
{
  uint32_t bytes = 0;
...
#if DEBUG_SE
FORCE_INLINE void DEBUG_print_byte_stream(const char *print_tag, ByteStream &b)
{
...
FORCE_INLINE void DEBUG_hex_dump_buf(const char *print_tag, const char *buf, int32_t len)
{
  const int32_t WRAP_COUNT = 16;
  for (int i = 0; i < len; i++) {
    if (i % WRAP_COUNT == 0) {
...
public:
  ...
  FORCE_INLINE static int write_i8(int8_t i8, ByteStream &out) { return write_ui8((uint8_t)i8, out); }
  FORCE_INLINE static int write_i16(int16_t i16, ByteStream &out) { return write_ui16((uint16_t)i16, out); }
  FORCE_INLINE static int write_i32(int32_t i32, ByteStream &out) { return write_ui32((uint32_t)i32, out); }
  FORCE_INLINE static int write_i64(int64_t i64, ByteStream &out) { return write_ui64((uint64_t)i64, out); }

  FORCE_INLINE static int read_i8(int8_t &i8, ByteStream &in) { return read_ui8((uint8_t&)i8, in); }
  FORCE_INLINE static int read_i16(int16_t &i16, ByteStream &in) { return read_ui16((uint16_t&)i16, in); }
  FORCE_INLINE static int read_i32(int32_t &i32, ByteStream &in) { return read_ui32((uint32_t&)i32, in); }
  FORCE_INLINE static int read_i64(int64_t &i64, ByteStream &in) { return read_ui64((uint64_t&)i64, in); }
  ...
  FORCE_INLINE static int do_write_var_uint(uint32_t ui32, char *out_buf, const uint32_t out_buf_len)
  {
    uint32_t offset = 0;
    while ((ui32 & 0xFFFFFF80) != 0) {
  ...
  FORCE_INLINE static int do_read_var_uint(uint32_t &ui32, char *in_buf, int in_buf_len, int *ret_offset)
  {
    ui32 = 0;
    int i = 0;
    uint8_t ui8 = 0;
  ...
  FORCE_INLINE static int write_var_uint(uint32_t ui32, ByteStream &out) { return do_write_var_uint(ui32, out); } 
  FORCE_INLINE static int write_var_uint(uint32_t ui32, char *out_buf, const uint32_t out_buf_len)
  {
    return do_write_var_uint(ui32, out_buf, out_buf_len);
  }
  FORCE_INLINE static int read_var_uint(uint32_t &ui32, ByteStream &in) { return do_read_var_uint(ui32, in); }
  FORCE_INLINE static int read_var_uint(uint32_t &ui32, char *in_buf, int in_buf_len, int *ret_offset = nullptr)
  {
  ...
  FORCE_INLINE static int read_mystring(String &str, common::PageArena *pa, ByteStream &in)
  {
    int ret = common::E_OK;
    int32_t len = 0;
    int32_t read_len = 0;
  ...
  FORCE_INLINE static int write_char(char ch, ByteStream &out) { return write_ui8(ch, out); }
  FORCE_INLINE static int read_char(char &ch, ByteStream &in)  { return read_ui8((uint8_t&)ch, in); }
};
