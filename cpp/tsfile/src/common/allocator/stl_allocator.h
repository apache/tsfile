...
public:
   ...
   */
  template<class U>
  struct rebind { typedef StlAllocator<U, Mid, TAllocator> other; };
  ...
  pointer allocate(size_type n, const void *hint = 0) { return (pointer)base_allocator_.alloc(n * sizeof(T), Mid); };
 ...
 * Although we define a base_allocator_ here, but base_allocator_ is also stateless.
 * so '==' is always true and '!=' is always false.
 * refer to https://en.cppreference.com/w/cpp/memory/allocator/operator_cmp.
 */
template<class T1, AllocModID Mid1, class T2, AllocModID Mid2>
bool operator==(const StlAllocator<T1, Mid1> &a1, const StlAllocator<T2, Mid2> &a2) { return true; }

template<class T1, AllocModID Mid1, class T2, AllocModID Mid2>
bool operator!=(const StlAllocator<T1, Mid1> &a1, const StlAllocator<T2, Mid2> &a2) { return false; }
