...
void *mem_alloc(uint32_t size, AllocModID mid)
{
  ...
  if (size <= 0xFFFFFF) {
    ...
    if (UNLIKELY(p == nullptr)) {
      std::cout<<"alloc filed for size " << size + HEADER_SIZE_4B << std::endl;
      return nullptr;
    } else {
    ...
    if (UNLIKELY(p == nullptr)) {
        std::cout<<"alloc big filed for size " << size + HEADER_SIZE_4B << std::endl;
      return nullptr;
    } else {
...
void mem_free(void *ptr)
{
  ...
  if ((header & 0x80) == 0) {
    ...
    // 8Byte header
    uint64_t header8b = ((uint64_t)(*(uint32_t*)(p - 4))) | ((uint64_t)(*(uint32_t*)(p - 8)) << 32);
...
void *mem_realloc(void *ptr, uint32_t size)
{
  ...
  uint32_t size_org;
  char *p = (char*)ptr;
  uint32_t header_org = *(uint32_t*)(p - HEADER_SIZE_4B);// try as 4Byte header
  if ((header_org & 0x80) == 0) {
    ...
    if (size <= 0xFFFFFF) {
      ...
      if (UNLIKELY(p == nullptr)) {
        ...
        uint32_t header = (size << 8) | ((uint32_t)mid_org); //size changed
        *((uint32_t*)p) = header;
        ModStat::get_instance().update_alloc(mid_org, int32_t(size) - int32_t(size_org));
        return p + HEADER_SIZE_4B;
      }
      ...
      if (UNLIKELY(p == nullptr)) { 
        ...
        uint64_t large_size = size;
        uint64_t header = ((large_size) << 8) | (((uint32_t)mid_org) | (0x80));
        uint32_t low4b = (uint32_t)(header & 0xFFFFFFFF);
        uint32_t high4b = (uint32_t)(header >> 32);
        *(uint32_t*)p = high4b;
        *(uint32_t*)(p + 4) = low4b;
    ...
    uint64_t header = ((uint64_t)(*(uint32_t*)(p - 4))) | ((uint64_t)(*(uint32_t*)(p - 8)) << 32); // 8Byte header
    mid_org = (AllocModID)(header & 0x7F);
    size_org = (uint32_t)(header >> 8);
    if (size <= 0xFFFFFF) {
      uint32_t save_data = *(uint32_t*)(p  - HEADER_SIZE_8B + HEADER_SIZE_4B + size);
      p = (char *) realloc(p - HEADER_SIZE_8B, size + HEADER_SIZE_4B);
      if (UNLIKELY(p == nullptr)) {
        return nullptr;
      } else {
        std::memmove(p + HEADER_SIZE_4B, p + HEADER_SIZE_8B, size - HEADER_SIZE_4B);
      ...
      if (UNLIKELY(p == nullptr)) {
        ...
        uint64_t large_size = size;
        uint64_t header8b = ((large_size) << 8) | (((uint32_t)mid_org) | (0x80));
