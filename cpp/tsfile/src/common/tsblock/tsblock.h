...
public:
   ...
   *            such as limit scenarios, such as based on statistical information,
   *            such as insert scenarios, etc.
   *            Then we will use the given number of rows
   */
  explicit TsBlock(TupleDesc *tupledesc, uint32_t max_row_count = 0) :
    capacity_(g_config_value_.tsblock_max_memory_),
    row_count_(0), max_row_count_(max_row_count), tuple_desc_(tupledesc) { }
  ...
  FORCE_INLINE uint32_t get_column_count() const { return tuple_desc_->get_column_count(); }
  ...
  FORCE_INLINE void update_capacity(uint32_t extend_size) { capacity_ += extend_size; }
  ...
  FORCE_INLINE int flush_row_count(uint32_t row_count)
  {
    ...
    if (row_count_ == 0) {
      row_count_ = row_count;
        ...
        log_error("Inconsistent number of rows in two columns");
...
private:
  ...
  void write_data(ByteStream* __restrict byte_stream,
                  char* __restrict val,
                  uint32_t len,
                  bool has_null,
                  TSDataType type);
...
public:
  ...
  FORCE_INLINE void append(uint32_t slot_index, const char *value, uint32_t len)
  {
...
public:
  ...
  FORCE_INLINE void append_null()
  {
    vec_->set_null(column_row_count_ - 1);
  }
...
public:
  ...
  FORCE_INLINE char* read(uint32_t slot_index, uint32_t* __restrict len, bool* __restrict null)
  {
...
private:
  TsBlock  *tsblock_;
  uint32_t row_id_;    // The line number currently being read
...
public:
  ...
  FORCE_INLINE char* read(uint32_t *len)
  {
    return vec_->read(len);
  }
