...
typedef struct FileID
{
  ...
  {
    out << "{seq_=" << file_id.seq_ << ", version_=" << file_id.version_ << ", merge_=" << file_id.merge_ << "}";
    return out;
  }
#endif
} FileID;
...
struct TsID
{
  ...
  {
    out << "(" << ti.db_nid_ << ", " << ti.device_nid_ << ", " << ti.measurement_nid_ << ")  ";
    return out;
  }

  FORCE_INLINE void to_string(char *print_buf, int len) const
  {
    snprintf(print_buf, len, "<%d,%d,%d>", db_nid_, device_nid_, measurement_nid_);
  }
  FORCE_INLINE std::string to_string() const 
  {
    const int buf_len = 32;
    char buf[buf_len];
    snprintf(buf, buf_len, "<%d,%d,%d>", db_nid_, device_nid_, measurement_nid_);
...
struct ColumnDesc
{
  ...
  void get_device_name(char *ret_device_name_buf,
                       const int buf_len,
  ...
  void get_measurement_name(char *ret_measurement_name_buf,
                            const int buf_len,
                            uint32_t &ret_len) const
  {
    int pos = column_name_.find_last_of('.');
    ...
    memcpy(ret_measurement_name_buf, column_name_.c_str() + pos + 1, ret_len);
...
{
  // todo: There may be a better way to avoid the memory problem of ostringstream
...
struct DatabaseIdTTL
{
  ...
  DatabaseIdTTL(const DatabaseIdTTL &other) :  db_nid_(other.db_nid_), ttl_(other.ttl_) {}
