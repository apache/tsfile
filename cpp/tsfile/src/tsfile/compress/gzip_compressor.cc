...
int GzipCompressor::init_zstream()
{
  ...
  if (deflateInit2(&compress_stream_,
    ...
    log_error("gzip deflateInit2 failed");
                                             ...
                                             ByteStream &out)
{
  ...
  if (uncompressed_buf == nullptr || uncompressed_buf_len == 0) {  // no more data will come in
    if (compress_stream_.next_out) {
      while (ret != Z_STREAM_END) {
        ...
        out.write_buf(compressed_buf, DEFLATE_BUFFER_SIZE - compress_stream_.avail_out);
        compress_stream_.next_out = (Bytef *)compressed_buf;
        compress_stream_.avail_out = DEFLATE_BUFFER_SIZE;
      }
    }
  ...
  for (;;) {
    ...
    if (compress_stream_.avail_in == 0) {  // current input data are all consumed
      out.write_buf(compressed_buf, DEFLATE_BUFFER_SIZE - compress_stream_.avail_out);
      compress_stream_.next_out = (Bytef *)compressed_buf;
      compress_stream_.avail_out = DEFLATE_BUFFER_SIZE;
      break;
                             ...
                             uint32_t &compressed_buf_len)
{
  ...
  if (RET_FAIL(compress_into_bytestream(uncompressed_buf, uncompressed_buf_len, out))) {
    return ret;
  }
  ...
  if (compressed_buf == nullptr || compressed_buf_len == 0) {
    if (decompress_stream_.next_out) {
      while (ret != Z_STREAM_END) {
        ...
        out.write_buf(decompressed_buf, INFLATE_BUFFER_SIZE - decompress_stream_.avail_out);
        decompress_stream_.next_out = (Bytef *)decompressed_buf;
        decompress_stream_.avail_out = INFLATE_BUFFER_SIZE;
      }
    }
                                 ...
                                 uint32_t &uncompressed_buf_len)
{
  ...
  if(RET_FAIL(decompress_into_bytestream(compressed_buf, compressed_buf_len, out))) {
    return ret;
  }
