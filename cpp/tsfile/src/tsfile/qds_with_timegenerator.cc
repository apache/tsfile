...
#include "tsfile/filter/time_filter.h"
#include "tsfile/filter/time_operator.h"
...
int64_t Node::get_cur_timestamp()
{
  ...
  if (type_ == AND_NODE) {
...
#if DEBUG_SE
      std::cout << dg.get_indent() << "Node::get_cur_timestamp: AND_NODE, lt=" << lt << ", rt=" << rt << std::endl;
#endif
    ...
    while (true) {
      ...
      int64_t rt = right_->get_cur_timestamp();
#if DEBUG_SE
      std::cout << dg.get_indent() << "Node::get_cur_timestamp:  OR_NODE, lt=" << lt << ", rt=" << rt << std::endl;
#endif
      if (lt == INVALID_NEXT_TIMESTAMP && rt == INVALID_NEXT_TIMESTAMP) {
...
#if DEBUG_SE
    std::cout << dg.get_indent() << "Node::get_cur_timestamp: SRS_NODE, res=" << res << std::endl;
#endif
    return res;
  }
}
...
void Node::next_timestamp(int64_t beyond_this_time)
{
  if (type_ == AND_NODE) {
#if DEBUG_SE
    std::cout << "next_timestamp. AND_NODE. beyond_this_time=" << beyond_this_time << std::endl;
#endif
    left_->next_timestamp(beyond_this_time);
    right_->next_timestamp(beyond_this_time);
  } else if (type_ == OR_NODE) {
#if DEBUG_SE
    std::cout << "next_timestamp.  OR_NODE. beyond_this_time=" << beyond_this_time << ", next_direction_=" << next_direction_ << std::endl;
#endif
...
int QDSWithTimeGenerator::init(TsFileIOReader *io_reader,
                               QueryExpression *qe)
{
  ...
  for (size_t i = 0; i < paths.size(); i++) {
    ValueAt va;
    if (RET_FAIL(io_reader_->alloc_ssi(paths[i].device_, paths[i].measurement_, va.ssi_))) {
    } else {
...
#if DEBUG_SE
  std::cout << "QDSWithTimeGenerator::get_next: timestamp=" << timestamp << ", will generate row at this timestamp." << std::endl;
#endif
