...
void OpenFile::set_file_id_and_path(const FileID &file_id, const std::string &file_path)
{
...
int OpenFile::build_from(const std::vector<TimeseriesTimeIndexEntry>& time_index_vec)
{
  MutexGuard mg(mutex_);
  ...
  for (size_t i = 0; i < time_index_vec.size(); i++) {
    ...
    std::pair<TsTimeRangeMapIterator, bool> ins_res = ts_time_range_map_->insert(pair);
    if (!ins_res.second) {
      return E_OOM;
    }
  }
...
int OpenFile::add(const TsID &ts_id, const TimeRange &time_range)
{
  ...
  if (find_iter != ts_time_range_map_->end()) {
    log_error("TimeseriesTimeIndex add dup timeseries %s", ts_id.to_string().c_str());
    ASSERT(false);
...
int OpenFile::get_time_range(const TsID &ts_id, TimeRange &ret_time_range) const
{
  int ret = E_OK;
  MutexGuard mg(mutex_);
  TsTimeRangeMapIterator find_iter = ts_time_range_map_->find(ts_id);
