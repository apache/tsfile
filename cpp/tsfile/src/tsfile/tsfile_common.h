...
#ifndef STORAGE_TSFILE_TSFILE_COMMON_H
#define STORAGE_TSFILE_TSFILE_COMMON_H
...
// Note, in tsfile_io_writer, we just write fields of PageHeader
// instead of do a serialize of PageHeader object.
// That is because statistic_ in PageHeader may be omitted if only
// one page exists in the chunk but we know that fact after we write
// the first page.
struct PageHeader
{
  ...
  PageHeader() : uncompressed_size_(0),
                 compressed_size_(0),
                 statistic_(nullptr) {}
  ~PageHeader() { reset(); }
  void reset()
  ...
  int deserialize_from(common::ByteStream &in, bool deserialize_stat, common::TSDataType data_type)
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::read_var_uint(uncompressed_size_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_var_uint(compressed_size_, in))) {
...
struct ChunkHeader
{
  ...
  int serialize_to(common::ByteStream &out)
  {
    ...
    } else if (RET_FAIL(common::SerializationUtil::write_str(measurement_name_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_var_uint(data_size_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_char(data_type_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_char(compression_type_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_char(encoding_type_, out))) {
    }
    return ret;
  }
  int deserialize_from(common::ByteStream &in)
  {
    ...
    } else if (RET_FAIL(common::SerializationUtil::read_str(measurement_name_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_var_uint(data_size_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_char((char&)data_type_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_char((char&)compression_type_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_char((char&)encoding_type_, in))) {
    } else {
  ...
  friend std::ostream& operator << (std::ostream &os, const ChunkHeader &h)
  {
    os << "{measurement_name=" << h.measurement_name_
      << ", data_size=" << h.data_size_
      << ", data_type=" << h.data_type_
...
struct ChunkMeta
{
  ...
  int init(const common::String &measurement_name,
           ...
           int64_t offset_of_chunk_header,
           Statistic *stat,
           const common::TsID &ts_id,
           char mask,
           common::PageArena &pa)
  {
  ...
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::write_i64(offset_of_chunk_header_, out))) {
    } else if (serialize_statistic) {
      ret = statistic_->serialize_to(out);
    }
    return ret;
  }
  int deserialize_from(common::ByteStream &in, bool deserialize_stat, common::PageArena *pa)
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::read_i64(offset_of_chunk_header_, in))) {
    } else if (deserialize_stat) {
      statistic_ = StatisticFactory::alloc_statistic_with_pa(data_type_, pa);
      if (IS_NULL(statistic_)) {
        ret = common::E_OOM;
...
public:
  ...
  static const common::AllocModID PAGE_ARENA_MOD_ID = common::MOD_TIMESERIES_INDEX_OBJ;
public:
  TimeseriesIndex()
    : timeseries_meta_type_((char)255),
      ...
      chunk_meta_list_serialized_buf_(CHUNK_META_LIST_SERIALIZED_BUF_PAGE_SIZE,
                                      PAGE_ARENA_MOD_ID),
  ...
  FORCE_INLINE int set_measurement_name(common::String &measurement_name, common::PageArena &pa)
  {
  ...
  FORCE_INLINE common::String get_measurement_name() { return measurement_name_; }
  common::SimpleList<ChunkMeta*>* get_chunk_meta_list() const { return chunk_meta_list_; }
  FORCE_INLINE void set_ts_meta_type(char ts_meta_type) { timeseries_meta_type_ = ts_meta_type; }
  FORCE_INLINE void set_data_type(common::TSDataType data_type) { data_type_ = data_type; }
  ...
  void set_ts_id(const common::TsID &ts_id)
  {
    ...
    if (chunk_meta_list_ != nullptr) {
      common::SimpleList<ChunkMeta*>::Iterator it = chunk_meta_list_->begin();
  ...
  {
    chunk_meta_list_data_size_ = chunk_meta_list_serialized_buf_.total_size();
  }

  int serialize_to(common::ByteStream &out)
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::write_char(timeseries_meta_type_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_mystring(measurement_name_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_char(data_type_, out))) {
    } else if (RET_FAIL(common::SerializationUtil::write_var_uint(chunk_meta_list_data_size_, out))) {
    } else if (RET_FAIL(statistic_->serialize_to(out))) {
    } else if (RET_FAIL(merge_byte_stream(out, chunk_meta_list_serialized_buf_))) {
    }
    return ret;
  }
  ...
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::read_char(timeseries_meta_type_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_mystring(measurement_name_, pa, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_char((char&)data_type_, in))) {
    } else if (RET_FAIL(common::SerializationUtil::read_var_uint(chunk_meta_list_data_size_, in))) {
    } else if (nullptr == (statistic_ = StatisticFactory::alloc_statistic_with_pa(data_type_, pa))) {
      ret = common::E_OOM;
    } else if (RET_FAIL(statistic_->deserialize_from(in))) {
    } else {
      ...
      const bool deserialize_chunk_meta_statistic = (timeseries_meta_type_ & 0x3F); // TODO
      chunk_meta_list_ = new (chunk_meta_list_buf) common::SimpleList<ChunkMeta*>(pa);
      uint32_t start_pos = in.read_pos();
      while (IS_SUCC(ret) && in.read_pos() < start_pos + chunk_meta_list_data_size_) {
        void *cm_buf = pa->alloc(sizeof(ChunkMeta));
        if (IS_NULL(cm_buf)) {
          ret = common::E_OOM;
        } else {
          ChunkMeta *cm = new (cm_buf) ChunkMeta;
          cm->measurement_name_.shallow_copy_from(this->measurement_name_);
          cm->data_type_ = this->data_type_;
          cm->mask_ = 0; // TODO
          if (RET_FAIL(cm->deserialize_from(in, deserialize_chunk_meta_statistic, pa))) {
  ...
  int clone_from(const TimeseriesIndex &that, common::PageArena *pa)
  {
    ...
    if (that.chunk_meta_list_ != nullptr) {
      ...
      for (it = that.chunk_meta_list_->begin(); IS_SUCC(ret) && it != that.chunk_meta_list_->end(); it++) {
        ChunkMeta *cm = it.get();
        void *cm_buf = pa->alloc(sizeof(ChunkMeta));
        if (IS_NULL(cm_buf)) {
          return common::E_OOM;
...
#ifndef NDEBUG
  friend std::ostream& operator << (std::ostream &os, const TimeseriesIndex &tsi)
  {
    ...
    if (tsi.chunk_meta_list_) {
      os << ", chunk_meta_list={";
      int count = 0;
      common::SimpleList<ChunkMeta*>::Iterator it = tsi.chunk_meta_list_->begin();
...
private:
  /*
   * If this timeseries has more than one chunk meta, timeseries_meta_type_ is 1.
   * Otherwise timeseries_meta_type_ is 0.
   * It also should OR with mask of chunk meta.
   */
  ...
  /*
   * If TimeseriesIndex has only one ChunkMeta, then TimeseriesIndex.statistic_ 
   * is duplicated with ChunkMeta.statistic_.
   * In this case, we do not serialize ChunkMeta.statistic_.
   */
  Statistic *statistic_;
...
{
public:
  explicit TSMIterator(common::SimpleList<ChunkGroupMeta*> &chunk_group_meta_list)
...
private:
  /*
  FORCE_INLINE bool is_same_measurement_name(const common::String &s1, const common::String &s2)
  {
...
struct MetaIndexEntry
{
  ...
  FORCE_INLINE int init(const std::string &str,
                        int64_t offset,
                        common::PageArena &pa)
  {
    offset_ = offset;
  ...
  int serialize_to(common::ByteStream &out)
  {
    ...
    } else if (RET_FAIL(common::SerializationUtil::write_i64(offset_, out))) {
    }
    return ret;
  }
...
#ifndef NDEBUG
  friend std::ostream& operator << (std::ostream &os, const MetaIndexEntry &entry)
  {
...
struct MetaIndexNode
{
  ...
  int binary_search_children(const common::String &name,
                             bool exact_search,
...
#endif
    if (RET_FAIL(common::SerializationUtil::write_var_uint(children_.size(), out))) {
    } else {
      ...
      if (IS_SUCC(ret)) {
        if (RET_FAIL(common::SerializationUtil::write_i64(end_offset_, out))) {
        } else if (RET_FAIL(common::SerializationUtil::write_char(node_type_, out))) {
        }
      }
    }
#if DEBUG_SE
    std::cout << "MetaIndexNode serialize_to. this=" << *this << " at file pos: " << start_pos << " to " << out.total_size() << std::endl;
#endif
    return ret;
  }
  ...
  int deserialize_from(common::ByteStream &in)
  {
    ...
    if (RET_FAIL(common::SerializationUtil::read_var_uint(children_size, in))) {
      return ret;
    }
    ...
    if (IS_SUCC(ret)) {
      char node_type_ch = 0;
      if (RET_FAIL(common::SerializationUtil::read_i64(end_offset_, in))) {
      } else if (RET_FAIL(common::SerializationUtil::read_char(node_type_ch, in))) {
      } else {
        node_type_ = (MetaIndexNodeType)node_type_ch;
      }
    }
#if DEBUG_SE
    std::cout << "MetaIndexNode deserialize_from. this=" << *this << std::endl;
#endif
    return ret;
  }

#ifndef NDEBUG
  friend std::ostream& operator << (std::ostream &os, const MetaIndexNode &node)
  {
  ...
  {
    return children_.size() >= common::g_config_value_.max_degree_of_index_node_;
  }
...
struct TsFileMeta
{
  ...
  TsFileMeta() : index_node_(nullptr),
                 meta_offset_(0),
  ...
  int serialize_to(common::ByteStream &out)
  {
    ...
    } else if (RET_FAIL(common::SerializationUtil::write_i64(meta_offset_, out))) {
    }
    return ret;
  }
  ...
  int deserialize_from(common::ByteStream &in)
  {
    ...
    } else if (RET_FAIL(common::SerializationUtil::read_i64(meta_offset_, in))) {
...
#ifndef NDEBUG
  friend std::ostream& operator << (std::ostream &os, const TsFileMeta &tsfile_meta)
  {
    os << "meta_offset=" << tsfile_meta.meta_offset_
       << ", index_node={" << *tsfile_meta.index_node_ << "}";
    return os;
  }
#endif
...
#endif // STORAGE_TSFILE_TSFILE_COMMON_H
