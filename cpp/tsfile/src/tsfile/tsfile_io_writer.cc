...
#if 1
#define OFFSET_DEBUG(msg) std::cout << "OFFSET_DEBUG: " << msg << ". offset=" << write_stream_.total_size() << std::endl 
#else
...
int TsFileIOWriter::init()
{
  ...
  if (RET_FAIL(file_->create(file_id, O_RDWR | O_CREAT, 0644))) {
    log_error("file open error, ret=%d", ret);
  }
  return ret;
}
#endif
...
{
    std::cout<<"begin to flush chunk group" << std::endl;
  int ret = E_OK;
                                      ...
                                      int32_t num_of_pages)
{
  ...
  return start_flush_chunk(chunk_data,
                           measurement_name,
                           col_desc.type_,
                           col_desc.encoding_,
                                      ...
                                      common::TsID ts_id)
{
  ...
  if (IS_NULL(buf1) || IS_NULL(buf2)) {
    ...
    Statistic *chunk_statistic_copy = placement_new_statistic(data_type, buf2);
    String mname((char*)measurement_name.c_str(), strlen(measurement_name.c_str()));
    ret = cur_chunk_meta_->init(mname, data_type,
                                cur_file_position(), chunk_statistic_copy,
                                ts_id, mask, meta_allocator_);
  }
  ...
  if (IS_SUCC(ret)) {
    ...
    chunk_header.chunk_type_ = (num_of_pages <= 1 ? ONLY_ONE_PAGE_CHUNK_HEADER_MARKER : CHUNK_HEADER_MARKER);
    OFFSET_DEBUG("start flush chunk header");
    ret = chunk_header.serialize_to(write_stream_);
    OFFSET_DEBUG("after flush chunk header");
#if DEBUG_SE
    std::cout << "TsFileIOWriter write ChunkHeader: " << chunk_header << std::endl;
#endif
  }
  return ret;
}
...
int TsFileIOWriter::flush_chunk(ByteStream &chunk_data)
{
  ...
  if (RET_FAIL(write_chunk_data(chunk_data))) {
    log_error("write chunk data error, ret=%d", ret);
  } else if (RET_FAIL(flush_stream_to_file())) {
    log_error("flush stream error, ret=%d", ret);
  }
...
{
  OFFSET_DEBUG("before write chunk data");
  // may print chunk_data here for debug.
#if DEBUG_SE
  DEBUG_print_byte_stream("WriteChunkData chunk_data=", chunk_data);
  std::cout << "WriteChunkData: write_stream_.total_size=" << write_stream_.total_size()
...
int TsFileIOWriter::end_file()
{
  ...
  if (RET_FAIL(write_log_index_range())) {
      std::cout<<"write range index error, ret =" << ret <<std::endl;
  } else if (RET_FAIL(write_file_index())) {
      std::cout<< "write file index error, ret = " << ret << std::endl;
  } else if (RET_FAIL(write_file_footer())) {
      std::cout<< "write file footer error, ret = " << ret << std::endl;
...
//  OFFSET_DEBUG("before write log index");
  if (RET_FAIL(write_byte(OPERATION_INDEX_RANGE))) {
      std::cout<<"write byte error " << ret << std::endl;
  } else if (RET_FAIL(SerializationUtil::write_i64(min_plan_index, write_stream_))) {
      std::cout<<"min index error " << ret << std::endl;
  } else if (RET_FAIL(SerializationUtil::write_i64(max_plan_index, write_stream_))) {
...
{
  std::cout << "ChunkGroupMeta = {device_name_=" << cgm->device_name_ << ", chunk_meta_list_={";
...
void debug_print_chunk_group_meta_list(common::SimpleList<ChunkGroupMeta*> &cgm_list)
{
...
int TsFileIOWriter::write_file_index()
{
  ...
  while (IS_SUCC(ret) && tsm_iter.has_next()) {
    ...
    if (RET_FAIL(tsm_iter.get_next(device_name, measurement_name, ts_index))) {
      break;
    } 
#if DEBUG_SE
    std::cout << "tsm_iter get next = {device_name=" << device_name << ", measurement_name=" << measurement_name << ", ts_index=" << ts_index << std::endl;
#endif

    // prepare if it is an entry of a new device
    if (!prev_device_name.equal_to(device_name)) {
      if (!prev_device_name.is_null()) {
        if (RET_FAIL(add_cur_index_node_to_queue(cur_index_node, cur_index_node_queue))) {
        } else if (RET_FAIL(add_device_node(device_map, prev_device_name, cur_index_node_queue, writing_mm))) {
        }
      }
      if(IS_SUCC(ret)) {
        if (RET_FAIL(alloc_meta_index_node_queue(writing_mm, cur_index_node_queue))) {
        } else if (RET_FAIL(alloc_and_init_meta_index_node(writing_mm, cur_index_node, LEAF_MEASUREMENT))) {
        }
      }
    ...
    if (IS_SUCC(ret) && entry_count_in_cur_device % g_config_value_.max_degree_of_index_node_ == 0) {
      ...
      if (IS_SUCC(ret)) {
        if (RET_FAIL(alloc_and_init_meta_index_entry(writing_mm, meta_index_entry, measurement_name))) {
        } else if (RET_FAIL(cur_index_node->push_entry(meta_index_entry))) {
        }
      }
    }
    ...
    if (IS_SUCC(ret)) {
      ...
      if (RET_FAIL(filter.add_path_entry(device_name, measurement_name))) {
      } else if (RET_FAIL(ts_index.serialize_to(write_stream_))) {
      } else {
#if DEBUG_SE
        std::cout << "ts_index.serialize. ts_index=" << ts_index << " file_pos=" << cur_file_position() << std::endl;
#endif
  ...
  if (IS_SUCC(ret)) {
    ...
    if (RET_FAIL(build_device_level(device_map, device_index_root_node, writing_mm))) {
    } else{
...
#if DEBUG_SE
      std::cout << "write tsfile_meta: "<< tsfile_meta << ", tsfile_meta_offset=" << tsfile_meta_offset << ", size=" << size << std::endl;
#endif
      tsfile_meta.index_node_ = nullptr; // memory management is delegated to writing_mm
    }
  }
  return ret;
}
...
int TsFileIOWriter::init_bloom_filter(BloomFilter &filter)
{
  ...
  if (RET_FAIL(filter.init(g_config_value_.tsfile_index_bloom_filter_error_percent_,
                           path_count))) {
  }
  return ret;
}

int32_t TsFileIOWriter::get_path_count(common::SimpleList<ChunkGroupMeta*> &cgm_list)
{
  ...
  for (; cgm_it != cgm_list.end(); cgm_it++) {
    ...
    for (; cm_it != cgm->chunk_meta_list_.end(); cm_it++) {
      ChunkMeta *cm = cm_it.get();
      // TODO currently, we do not have multi chunks of same timeseries in tsfile.
                                       ...
                                       FileIndexWritingMemManager &wmm)
{
  ...
  SimpleList<MetaIndexNode*> node_queue(1024, MOD_TSFILE_WRITER_META); // FIXME
  DeviceNodeMapIterator device_map_iter;

  MetaIndexNode *cur_index_node = nullptr;
  if (RET_FAIL(alloc_and_init_meta_index_node(wmm, cur_index_node, LEAF_DEVICE))) {
    return ret;
  }
...
#if DEBUG_SE
      std::cout << "TsFileIOWriter::build_device_level, cur_index_node=" << *cur_index_node << std::endl;
#endif
      if (RET_FAIL(node_queue.push_back(cur_index_node))) {
      } else if (RET_FAIL(alloc_and_init_meta_index_node(wmm, cur_index_node, LEAF_DEVICE))) {
      }
    }
    if (RET_FAIL(alloc_and_init_meta_index_entry(wmm, entry, device_name))) {
    } else if (RET_FAIL(device_map_iter->second->serialize_to(write_stream_))) {
  ...
  if (IS_SUCC(ret)) {
    if (node_queue.size() > 0) {
      if (RET_FAIL(generate_root(&node_queue, ret_root, INTERNAL_DEVICE, wmm))) {
      }
    } else {
...
int TsFileIOWriter::alloc_and_init_meta_index_entry(FileIndexWritingMemManager &wmm,
...
#if DEBUG_SE
  std::cout << "alloc_and_init_meta_index_entry, MetaIndexEntry=" << *ret_entry << std::endl;
#endif
  return E_OK;
}

int TsFileIOWriter::alloc_and_init_meta_index_node(FileIndexWritingMemManager &wmm,
...
#if DEBUG_SE
  std::cout << "alloc_and_init_meta_index_node, node=" << *ret_node << std::endl;
#endif
  return E_OK;
}

int TsFileIOWriter::add_cur_index_node_to_queue(MetaIndexNode *node,
                                                SimpleList<MetaIndexNode*> *queue)
{
  node->end_offset_ = cur_file_position();
#if DEBUG_SE
  std::cout << "add_cur_index_node_to_queue, node=" << (void*)node << " = " << *node << ", set offset=" << cur_file_position() << std::endl;
#endif
  return queue->push_back(node);
}

int TsFileIOWriter::alloc_meta_index_node_queue(FileIndexWritingMemManager &wmm,
                                                SimpleList<MetaIndexNode*> *&queue)
{
...
int TsFileIOWriter::add_device_node(DeviceNodeMap &device_map,
                                    String device_name,
                                    ...
                                    FileIndexWritingMemManager &wmm)
{
  ...
  if (RET_FAIL(generate_root(measurement_index_node_queue, root, INTERNAL_MEASUREMENT, wmm))) {
  } else {
    std::pair<String, MetaIndexNode*> ins_pair;
    ins_pair.first.shallow_copy_from(device_name);
    ins_pair.second = root;
  
    std::pair<DeviceNodeMapIterator, bool> ins_res = device_map.insert(ins_pair);
    if (!ins_res.second) {
      ASSERT(false);
    }
  }
                                  ...
                                  FileIndexWritingMemManager &wmm)
{
  ...
  MetaIndexNode *cur_index_node = nullptr;
  if (RET_FAIL(alloc_and_init_meta_index_node(wmm, cur_index_node, node_type))) {
  }
  uint32_t from_size = from->size();
  while (IS_SUCC(ret)) {
    to->clear();
    SimpleList<MetaIndexNode*>::Iterator from_iter;
    for (from_iter = from->begin(); IS_SUCC(ret) && from_iter != from->end(); from_iter++) {
      MetaIndexNode *iter_node = from_iter.get();
      MetaIndexEntry *entry = nullptr;
      String name;
      if (RET_FAIL(iter_node->get_first_child_name(name))) {
      } else if (RET_FAIL(alloc_and_init_meta_index_entry(wmm, entry, name))) {
      } else if (cur_index_node->is_full()) {
        cur_index_node->end_offset_ = cur_file_position();
        if (RET_FAIL(to->push_back(cur_index_node))) {
        } else {
#if DEBUG_SE
          std::cout << "generate root, alloc_and_init_meta_index_node. cur_index_node=" << *cur_index_node << std::endl;
#endif
          if (RET_FAIL(alloc_and_init_meta_index_node(wmm, cur_index_node, node_type))) {
          }
        }
      ...
      if (IS_SUCC(ret)) {
        if (RET_FAIL(cur_index_node->push_entry(entry))) {
          ...
          OFFSET_DEBUG("before write index_node in generate_root");
          ret = iter_node->serialize_to(write_stream_);
          OFFSET_DEBUG("after write index_node in generate_root");
        }
      }
    } // end for
    if (IS_SUCC(ret)) {
...
#if DEBUG_SE
        std::cout << "genereate root 2, alloc_and_init_meta_index_node. cur_index_node=" << *cur_index_node << std::endl;
#endif
        if (RET_FAIL(alloc_and_init_meta_index_node(wmm, cur_index_node, node_type))) {
        }
      }
...
int TsFileIOWriter::flush_stream_to_file()
{
  int ret = E_OK;
  while (true) {
    ByteStream::Buffer b = write_stream_consumer_.get_next_buf(write_stream_);
    if (b.buf_ == nullptr) {
      break;
    } else {
...
void TsFileIOWriter::add_ts_time_index_entry(TimeseriesIndex &ts_index)
{
  ...
  time_index_entry.time_range_.start_time_ = ts_index.get_statistic()->start_time_;
  time_index_entry.time_range_.end_time_ = ts_index.get_statistic()->end_time_;
