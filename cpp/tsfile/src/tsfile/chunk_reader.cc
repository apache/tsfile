...
#include "tsfile/encode/decoder_factory.h"
#include "tsfile/compress/compressor_factory.h"
...
int ChunkReader::init(ReadFile *read_file,
                      String m_name,
...
int ChunkReader::load_by_meta(ChunkMeta *meta)
{
  ...
  // at least, we can read the chunk header and the first page header.
  // TODO configurable
  file_data_buf_size_ = 1024;
  int32_t ret_read_len = 0;
  char *file_data_buf = (char*)mem_alloc(file_data_buf_size_, MOD_CHUNK_READER);
  ...
  if (IS_SUCC(ret) && ret_read_len < ChunkHeader::MIN_SERIALIZED_SIZE) {
    ...
    log_error("file corrupted, ret=%d, offset=%d, read_len=%d",
              ret, chunk_meta_->offset_of_chunk_header_, ret_read_len);
  }

  /* ================ Step 2: deserialize chunk_header ================*/
  if (IS_SUCC(ret)) {
    in_stream_.wrap_from(file_data_buf, ret_read_len);
    // std::cout << "in_stream_.wrap_from " << (void*)file_data_buf << std::endl;
    if (RET_FAIL(chunk_header_.deserialize_from(in_stream_))) {
    } else if (RET_FAIL(alloc_compressor_and_value_decoder(chunk_header_.encoding_type_,
            chunk_header_.data_type_, chunk_header_.compression_type_))) {
    } else {
      chunk_visit_offset_ = in_stream_.read_pos(); // point to end of chunk_header_
#if DEBUG_SE
      std::cout << "ChunkReader::load_by_meta, chunk_header=" << chunk_header_ << std::endl;
#endif
    }
  }
  return ret;
...
int ChunkReader::alloc_compressor_and_value_decoder(TSEncoding encoding,
                                                    TSDataType data_type,
...
int ChunkReader::get_next_page(TsBlock *ret_tsblock, Filter *oneshoot_filter)
{
  ...
  if (prev_page_not_finish()) {
    ret = decode_tv_buf_into_tsblock_by_datatype(time_in_, value_in_, ret_tsblock, filter);
    if (ret == E_OVERFLOW) {
      ret = E_OK;
    } else {
...
int ChunkReader::get_cur_page_header()
{
  ...
  do {
    ...
    cur_page_header_.reset();
    ret = cur_page_header_.deserialize_from(in_stream_,
                                            !chunk_has_only_one_page(),
                                            chunk_header_.data_type_);
...
#if DEBUG_SE
  std::cout << "get_cur_page_header, ret=" << ret
            << ", retry=" << retry
            << ", cur_page_header_=" << cur_page_header_
            << ", chunk_meta_->offset_of_chunk_header_=" << chunk_meta_->offset_of_chunk_header_
            << ", cur_page_header_serialized_size=" << cur_page_header_serialized_size << std::endl;
#endif
  return ret;
}

// read at least @want_size bytes from file and wrap the buffer into @in_stream_
int ChunkReader::read_from_file_and_rewrap(int want_size)
{
  ...
  int read_size = (want_size < DEFAULT_READ_SIZE ? DEFAULT_READ_SIZE : want_size);
  if (file_data_buf_size_ < read_size || read_size < file_data_buf_size_ / 10) {
  ...
  int ret_read_len = 0;
  if (RET_FAIL(read_file_->read(offset,
                                file_data_buf,
                                DEFAULT_READ_SIZE,
                                ret_read_len))) {
  } else {
...
int ChunkReader::decode_cur_page_data(TsBlock *&ret_tsblock, Filter *filter)
{
  ...
  if (in_stream_.remaining_size() < cur_page_header_.compressed_size_) {
    // std::cout << "decode_cur_page_data. in_stream_.remaining_size="<< in_stream_.remaining_size() << ", cur_page_header_.compressed_size_=" << cur_page_header_.compressed_size_ << std::endl;
    if (RET_FAIL(read_from_file_and_rewrap(cur_page_header_.compressed_size_))) {
    }
  }
  ...
  if (IS_SUCC(ret)) {
    ...
    // std::cout << "ChunkReader::decode_cur_page_data, in_stream_.get_wrapped_buf=" <<(void*)(in_stream_.get_wrapped_buf())<< ", in_stream_.read_pos=" << in_stream_.read_pos() << std::endl;
    ...
    } else if (RET_FAIL(compressor_->uncompress(compressed_buf, compressed_buf_size,
                                  uncompressed_buf, uncompressed_buf_size))) {
    } else {
      uncompressed_buf_ = uncompressed_buf;
    }
    // DEBUG_hex_dump_buf("ChunkReader read, uncompressed buf = ", uncompressed_buf, uncompressed_buf_size);
    if (ret != E_OK || uncompressed_buf_size != cur_page_header_.uncompressed_size_) {
      ret = E_TSFILE_CORRUPTED;
      ASSERT(false);
    }
  }
  ...
  if (IS_SUCC(ret)) {
    int var_size = 0;
    if (RET_FAIL(SerializationUtil::read_var_uint(time_buf_size, uncompressed_buf,
                                                  uncompressed_buf_size, &var_size))) {
    } else {
...
#if DEBUG_SE
    std::cout << "ChunkReader uncompress: compressed_buf_size=" << compressed_buf_size
  ...
  if (IS_SUCC(ret)) {
    ...
    //                                  value_buf_size, ret_tsblock, filter);
    ret = decode_tv_buf_into_tsblock_by_datatype(time_in_, value_in_, ret_tsblock, filter);
...
#define DECODE_TYPED_TV_INTO_TSBLOCK(CppType, ReadType, time_in, value_in, row_appender)  \
  do {                                                                                    \
    int64_t time = 0;                                                                     \
    CppType value;                                                                        \
    while (time_decoder_->has_remaining() || time_in.has_remaining()) {                   \
      ...
      } else if (RET_FAIL(value_decoder_->read_##ReadType(value, value_in))) {            \
      } else if (filter != nullptr && !filter->satisfy(time, value)) {                    \
        row_appender.backoff_add_row();                                                   \
        continue;                                                                         \
      } else {                                                                            \
        /*std::cout << "decoder: time=" << time << ", value=" << value << std::endl;*/    \
                                                        ...
                                                        Filter *filter)
{
  ...
  switch (chunk_header_.data_type_) {
    case timecho::common::BOOLEAN:
      DECODE_TYPED_TV_INTO_TSBLOCK(bool, boolean, time_in_, value_in_, row_appender);
      break;
    case timecho::common::INT32:
      // DECODE_TYPED_TV_INTO_TSBLOCK(int32_t, int32, time_in_, value_in_, row_appender);
      ret = i32_DECODE_TYPED_TV_INTO_TSBLOCK(time_in_, value_in_, row_appender, filter);
      break;
    case timecho::common::INT64:
      DECODE_TYPED_TV_INTO_TSBLOCK(int64_t, int64, time_in_, value_in_, row_appender);
      break;
    case timecho::common::FLOAT:
      DECODE_TYPED_TV_INTO_TSBLOCK(float, float, time_in_, value_in_, row_appender);
      break;
    case timecho::common::DOUBLE:
      DECODE_TYPED_TV_INTO_TSBLOCK(double, double, time_in_, value_in_, row_appender);
      break;
    default:
      ret = E_NOT_SUPPORT;
      ASSERT(false);
