...
#ifndef STORAGE_TSFILE_TSFILE_IO_WRITER_H
#define STORAGE_TSFILE_TSFILE_IO_WRITER_H
...
{
public:
  typedef std::map<common::String, MetaIndexNode*, common::StringLessThan> DeviceNodeMap;
  typedef DeviceNodeMap::iterator DeviceNodeMapIterator;
public:
  static const uint32_t WRITE_STREAM_PAGE_SIZE = 512; // FIXME
public:
  TsFileIOWriter() : meta_allocator_(),
                     write_stream_(WRITE_STREAM_PAGE_SIZE,
                                   common::MOD_TSFILE_WRITE_STREAM,
                                   /*atomic*/true),
  ...
  FORCE_INLINE std::vector<TimeseriesTimeIndexEntry>& get_ts_time_index_vector()
  {
...
private:
  ...
  FORCE_INLINE int64_t cur_file_position() const { return write_stream_.total_size(); }
  ...
  {
     std::cout<<"begin to write byte" << byte << std::endl;
    return common::SerializationUtil::write_char(byte, write_stream_);
  }
  FORCE_INLINE int write_string(const std::string &str)
  {
    int ret = common::E_OK;
    if (RET_FAIL(common::SerializationUtil::write_var_int(str.size(), write_stream_))) {
  ...
  int alloc_meta_index_node_queue(FileIndexWritingMemManager &wmm,
                                  common::SimpleList<MetaIndexNode*> *&queue);
  int add_device_node(DeviceNodeMap &device_map,
                      common::String device_name,
...
#endif // STORAGE_TSFILE_TSFILE_IO_WRITER_H
