...
#ifndef STORAGE_CHUNK_WRITER_H
#define STORAGE_CHUNK_WRITER_H
...
public:
  ChunkWriter() : data_type_(common::VECTOR),
                  page_writer_(),
  ...
  int init(const std::string &measurement_name,
           common::TSDataType data_type,
           common::TSEncoding encoding,
           common::CompressionType compression_type);
  void destroy();

  FORCE_INLINE int write(int64_t timestamp, bool    value) { CW_DO_WRITE_FOR_TYPE(common::BOOLEAN); }
  FORCE_INLINE int write(int64_t timestamp, int32_t value) { CW_DO_WRITE_FOR_TYPE(common::INT32);   }
  FORCE_INLINE int write(int64_t timestamp, int64_t value) { CW_DO_WRITE_FOR_TYPE(common::INT64);   }
  FORCE_INLINE int write(int64_t timestamp, float   value) { CW_DO_WRITE_FOR_TYPE(common::FLOAT);   }
  FORCE_INLINE int write(int64_t timestamp, double  value) { CW_DO_WRITE_FOR_TYPE(common::DOUBLE);  }
  ...
  FORCE_INLINE bool is_full() const
  {
    ...
    // In Java IoTDB, compaction may generate multi chunks (for reuse chunk etc)
    return false;
  }
