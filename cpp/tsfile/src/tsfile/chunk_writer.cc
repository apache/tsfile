...
{
  return init(col_desc.column_name_, col_desc.type_, col_desc.encoding_, col_desc.compression_);
}

int ChunkWriter::init(const std::string &measurement_name,
                      ...
                      CompressionType compression_type)
{
  ...
  if (chunk_statistic_ == nullptr) {
    return E_OOM;
  } else if (RET_FAIL(page_writer_.init(data_type, encoding, compression_type))) {
  } else if (IS_NULL((first_page_statistic_ = StatisticFactory::alloc_statistic(data_type)))) {
    ret = E_OOM;
  } else {
...
int ChunkWriter::seal_cur_page(bool end_chunk)
{
  ...
  if (num_of_pages_ == 0) {
    if (end_chunk) {
      ...
      ret = page_writer_.write_to_chunk(chunk_data_, /*header*/true, /*stat*/false, /*data*/true);
      page_writer_.destroy_page_data();
      page_writer_.destroy();
    } else {
      /*
       * if the chunk has only one page, do not write page statistic.
       * so we save the data of first page and see if the chunk has more page later.
       */
      ret = page_writer_.write_to_chunk(chunk_data_, /*header*/true, /*stat*/false, /*data*/false);
      if (IS_SUCC(ret)) {
    ...
    if (num_of_pages_ == 1) {
      // the chunk has more than one page, write first page now
      if (RET_FAIL(write_first_page_data(chunk_data_))) {
      }
      free_first_writer_data();
    }
    if (IS_SUCC(ret)) {
      if (RET_FAIL(page_writer_.write_to_chunk(chunk_data_, /*header*/true, /*stat*/true, /*data*/true))) {
      }
      page_writer_.destroy_page_data();
      page_writer_.reset();
    }
  ...
  std::cout << "seal_cur_page, num_of_pages_=" <<  num_of_pages_
            << ", end_chunk=" << end_chunk
            << ". After seal: chunk.get_point_numer()=" << chunk_statistic_->count_
            << ", chunk_data.size=" << chunk_data_.total_size()
            << std::endl;
#endif
  return ret;
}
...
int ChunkWriter::write_first_page_data(ByteStream &pages_data)
{
  ...
  } else if (RET_FAIL(pages_data.write_buf(first_page_data_.compressed_buf_,
...
int ChunkWriter::end_encode_chunk()
{
  ...
  std::cout << "end_encode_chunk: num_of_pages_=" << num_of_pages_
            ...
            << ", page_writer.get_statistic()->count_=" <<  page_writer_.get_statistic()->count_
            << std::endl;
#endif
  return ret;
}
