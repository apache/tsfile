#ifndef STORAGE_TSFILE_ENCODE_ZIGZAG_DECODER_H
#define STORAGE_TSFILE_ENCODE_ZIGZAG_DECODER_H
  ...
  void splice_bytes_in_arr()
  {
    ...
    if (num_of_sorts_of_zigzag_ == 1) {
      ...
      for (int i = 0; i < num_of_sorts_of_zigzag_; i++) {
        uint64_t value_shift = (uint64_t) (zigzag_decode_arr_[i]) << (int)(7*i);
...
int32_t ZigzagDecoder<int32_t>::decode(common::ByteStream &in)
{
  if (UNLIKELY(first_read_ == true)) {
    read_header(in);
    zigzag_decode_arr_ = (uint8_t*)common::mem_alloc(10, common::MOD_ZIGZAG_OBJ);
    buffer_ = 0;
    first_read_ = false;
...
#endif // STORAGE_TSFILE_ENCODE_zigzag_DECODER_H
