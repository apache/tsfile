...
#ifndef STORAGE_TSFILE_ENCODE_BITPACK_ENCODER_H
#define STORAGE_TSFILE_ENCODE_BITPACK_ENCODER_H
...
public:
  ...
  BitPackEncoder() :  bitpacked_group_count_(0), 
  ...
  void convert_buffer() 
  {
    ...
    unsigned char *bytes = (unsigned char *)common::mem_alloc(bit_width_, common::MOD_BITENCODE_OBJ);
    int tmp_buffer[8];
    for (int i = 0; i < 8; i++) {
  ...
  {
    //TODO: Optimization - find the maximum value first, and then calcuate the bit width
    int max = 1;
  ...
  {
    unsigned char bitPackHeader = (unsigned char) ((bitpacked_group_count_ << 1) | 1);
    common::SerializationUtil::write_ui8(bitPackHeader, byte_cache_);
    common::SerializationUtil::write_ui8((uint8_t)last_bitpacked_num, byte_cache_);
