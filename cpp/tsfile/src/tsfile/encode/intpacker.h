...
#ifndef STORAGE_TSFILE_ENCODE_IntPacker_ENCODER_H
#define STORAGE_TSFILE_ENCODE_IntPacker_ENCODER_H
  ...
  void pack_8values(int values[] , int offset, unsigned char buf[]) 
  {
    ...
    while (value_idx < NUM_OF_INTS + offset) {
      ...
      if (left_size > 0 && value_idx < NUM_OF_INTS + offset) {
        // put the first 'left_size' bits of the Integer into remaining space of the
        // buffer
      ...
      for (int j = 0; j < 4; j++) {
        buf[buf_idx] = (unsigned char) (((unsigned)buffer >> ((3 - j) * 8)) & 0xFF);
        buf_idx++;
        // width_ is the bit num of each value, but here is means the max byte num
        if (buf_idx >= width_) {
          return;
        }
      }
...
#endif // STORAGE_TSFILE_ENCODE_IntPacker_ENCODER_H
