                              ...
                              Filter *time_filter)
{
  ...
  if (RET_FAIL(load_tsfile_meta_if_necessary())) {
    ...
    if (RET_FAIL(load_timeseries_index_for_ssi(device_path, measurement_name, ssi))) {
...
bool TsFileIOReader::filter_stasify(TimeseriesIndex &ts_index, Filter *time_filter)
{
...
int TsFileIOReader::load_tsfile_meta_if_necessary()
{
  ...
  if (!tsfile_meta_ready_) {
    if (RET_FAIL(load_tsfile_meta())) {
      log_error("load_tsfile_meta error, ret=%d", ret);
      return ret;
    } else {
...
int TsFileIOReader::load_tsfile_meta()
{
  ...
  const int32_t TAIL_MAGIC_AND_META_SIZE_SIZE = 10; // magic(6B) + meta_size(4B)
  ...
  // Step 1: read the tsfile_meta_size
  // 1.1 prepare read buffer
  ...
  ret_read_len = 0;
  if (RET_FAIL(read_file_->read(read_offset, read_buf, alloc_size, ret_read_len))) {
  } else if (ret_read_len != alloc_size) {
    ret = E_FILE_READ_ERR;
    log_error("do not read enough data from tsfile, want-size=%d, read-size=%d, file=%s",
              alloc_size, ret_read_len, get_file_path().c_str());
  }
  ...
  // Step 2: read TsFileMeta
  if (IS_SUCC(ret)) {
    // 2.1 prepare enough buffer (use the previous buffer if can).
    char *tsfile_meta_buf = nullptr;
    if (tsfile_meta_size + TAIL_MAGIC_AND_META_SIZE_SIZE > (uint32_t)alloc_size) {
      // prepare buffer to re-read from start of tsfile_meta
      char *old_read_buf = read_buf;
      read_buf = (char*)mem_realloc(read_buf, tsfile_meta_size);
      if (IS_NULL(read_buf)) {
        read_buf = old_read_buf;
        ret = E_OOM;
      } else if (RET_FAIL(read_file_->read(file_size() - tsfile_meta_size - TAIL_MAGIC_AND_META_SIZE_SIZE, read_buf, tsfile_meta_size, ret_read_len))) {
      } else if (tsfile_meta_size != (uint32_t)ret_read_len) {
        ret = E_FILE_READ_ERR;
        log_error("do not read enough data from tsfile, want-size=%d, read-size=%d, file=%s",
                  tsfile_meta_size, ret_read_len, get_file_path().c_str());
      } else {
        tsfile_meta_buf = read_buf;
      }
    } else { // the previous buffer has contained the TsFileMeta data
      tsfile_meta_buf = read_buf + alloc_size - tsfile_meta_size - TAIL_MAGIC_AND_META_SIZE_SIZE;
      // DEBUG_hex_dump_buf("tsfile_meta_buf=", tsfile_meta_buf, tsfile_meta_size);
    }
    if (IS_SUCC(ret)) {
...
#if DEBUG_SE
      std::cout << "load tsfile_meta, ret=" << ret << ", tsfile_meta_=" << tsfile_meta_ << std::endl;
#endif
    }
  }
  mem_free(read_buf);
...
int TsFileIOReader::load_timeseries_index_for_ssi(const std::string &device_path,
                                                  const std::string &measurement_name,
                                                  TsFileSeriesScanIterator *&ssi)
{
  int ret = E_OK;
  ...
  if (RET_FAIL(load_device_index_entry(device_path,
                                       device_index_entry,
                                       device_ie_end_offset))) {
  } else if (RET_FAIL(load_measurement_index_entry(measurement_name,
                                                   device_index_entry.offset_,
                                                   device_ie_end_offset,
                                                   measurement_index_entry,
                                                   measurement_ie_end_offset))) {
  } else if (RET_FAIL(do_load_timeseries_index(measurement_name,
                                               measurement_index_entry.offset_,
                                               measurement_ie_end_offset,
                                               ssi->timeseries_index_pa_,
                                               ssi->timeseries_index_))) {
  } else {
#if DEBUG_SE
    std::cout << "load timeseries index: " << ssi->timeseries_index_ << std::endl;
#endif
  }
  return ret;
}
                                            ...
                                            int64_t &end_offset)
{
  ...
  if (index_node->node_type_ == LEAF_DEVICE) {
    // FIXME
    ret = index_node->binary_search_children(device_name, true, device_index_entry, end_offset);
  } else {
    ret = search_from_internal_node(device_name, index_node, device_index_entry, end_offset);
  }
  if (ret == E_NOT_EXIST) {
    ret = E_DEVICE_NOT_EXIST;
  }
#if DEBUG_SE
  std::cout << "load_device_index_entry, device_index_entry={" << device_index_entry
            << "}, end_offset=" << end_offset << std::endl;
#endif
  return ret;
}

int TsFileIOReader::load_measurement_index_entry(const std::string &measurement_name_str,
...
{
#if DEBUG_SE
  std::cout << "load_measurement_index_entry: measurement_name_str=" << measurement_name_str << ", start_offset=" << start_offset << ", end_offset=" << end_offset << std::endl;
#endif
  ...
  if (RET_FAIL(read_file_->read(start_offset, data_buf, read_size, ret_read_len))) {
  } else if (RET_FAIL(top_node->deserialize_from(data_buf, read_size))) {
  }
#if DEBUG_SE
  std::cout << "load_measurement_index_entry deserialize MetaIndexNode, top_node=" << *top_node << " at file pos " << start_offset << " to " << end_offset << std::endl;
#endif
  ...
  if (IS_SUCC(ret)) {
    ...
    if (top_node->node_type_ == LEAF_MEASUREMENT) {
      ret = top_node->binary_search_children(measurement_name, /*exact*/ false,
          ret_measurement_index_entry, ret_end_offset);
    } else {
      ret = search_from_internal_node(measurement_name, top_node, ret_measurement_index_entry,
          ret_end_offset); 
    }
  }
  if (ret == E_NOT_EXIST) {
...
{
  int ret = E_OK;
  ret = index_node->binary_search_children(target_name, true, ret_index_entry, ret_end_offset);
  return ret;
}
                                              ...
                                              int64_t &ret_end_offset)
{
  ...
  ASSERT(index_node->node_type_ == INTERNAL_MEASUREMENT || index_node->node_type_ == INTERNAL_DEVICE);

  if (RET_FAIL(index_node->binary_search_children(target_name, /*exact=*/false, index_entry, end_offset))) {
    return ret;
  }
  ...
  while (IS_SUCC(ret)) {
...
#if DEBUG_SE
    std::cout << "search_from_internal_node, end_offset=" << end_offset << ", index_entry.offset_=" << index_entry.offset_ << std::endl;
#endif
    ...
    MetaIndexNode *cur_level_index_node = new (buf) MetaIndexNode(&cur_level_index_node_pa);
    int32_t ret_read_len = 0;
    if (RET_FAIL(read_file_->read(index_entry.offset_, data_buf, read_size, ret_read_len))) {
    } else if (read_size != ret_read_len) {
      ret = E_TSFILE_CORRUPTED;
    } else if (RET_FAIL(cur_level_index_node->deserialize_from(data_buf, read_size))) {
    } else {
      if (cur_level_index_node->node_type_ == LEAF_DEVICE) {
        ret = cur_level_index_node->binary_search_children(target_name, /*exact=*/ true, ret_index_entry, ret_end_offset); 
        cur_level_index_node->destroy();
        return ret; //// FIXME
      } else if (cur_level_index_node->node_type_ == LEAF_MEASUREMENT) {
        ret = cur_level_index_node->binary_search_children(target_name, /*exact=*/ false, ret_index_entry, ret_end_offset); 
        cur_level_index_node->destroy();
        return ret; //// FIXME
      } else {
        ret = cur_level_index_node->binary_search_children(target_name, /*exact=*/ false, index_entry, end_offset);
        cur_level_index_node->destroy();
      }
    }
  }
...
int TsFileIOReader::do_load_timeseries_index(const std::string &measurement_name_str,
                                             int64_t start_offset,
                                             int64_t end_offset,
                                             PageArena &in_timeseries_index_pa,
                                             TimeseriesIndex &ret_timeseries_index)
{
  ...
  if (RET_FAIL(read_file_->read(start_offset, ti_buf, read_size, ret_read_len))) {
  } else {
    ByteStream bs;
    bs.wrap_from(ti_buf, read_size);
    const String target_measurement_name((char*)measurement_name_str.c_str(),
                                         strlen(measurement_name_str.c_str()));
    bool found = false;
#if DEBUG_SE
    std::cout << "do_load_timeseries_index, read file at " << start_offset << " to " << end_offset << std::endl;
#endif
    while (IS_SUCC(ret)) {
      ...
      if (RET_FAIL(cur_timeseries_index.deserialize_from(bs, &cur_timeseries_index_pa))) {
      } else if (cur_timeseries_index.get_measurement_name().equal_to(target_measurement_name)) {
        // cur_timeseries_index.set_ts_id(col_desc.ts_id_);
        if (RET_FAIL(ret_timeseries_index.clone_from(cur_timeseries_index, &in_timeseries_index_pa))) {
          log_error("timeseries index clone error, ret=%d", ret);
        }
        found = true;
        break;
      }
                                                       ...
                                                       const std::string &measurement_name)
{
  ...
  if (col_desc.ts_id_ != timeseries_index_.get_ts_id()) {
    if (RET_FAIL(load_timeseries_index(col_desc))) {
      log_error("load timeseries_index error, ret=%d", ret);
    } else {
