...
#ifndef STORAGE_TSFILE_PAGE_WRITER_H
#define STORAGE_TSFILE_PAGE_WRITER_H

#include "common/allocator/byte_stream.h"
#include "common/db_utils.h"
#include "statistic.h"
#include "tsfile/encode/encoder.h"
#include "tsfile/compress/compressor.h"
 ...
 *
 * We abstract this struct because chunk writer will delay write
...
struct PageData
{
  ...
  PageData() : time_buf_size_(0),
               ...
               compressor_(nullptr) {}
  int init(common::ByteStream &time_bs,
           common::ByteStream &value_bs,
...
#define PW_DO_WRITE_FOR_TYPE(TSDATATYPE)                                        \
  {                                                                             \
    int ret = common::E_OK;                                                     \
    /* std::cout << "page_writer write: time=" << timestamp << ", value=" << value << std::endl; */ \
    if (UNLIKELY(data_type_ != TSDATATYPE)) {                                   \
      ret = common::E_TYPE_NOT_MATCH;                                           \
    } else if (RET_FAIL(time_encoder_->encode(timestamp, time_out_stream_))) {  \
    } else if (RET_FAIL(value_encoder_->encode(value, value_out_stream_))) {    \
    } else {                                                                    \
...
{
public:
  PageWriter() : data_type_(common::VECTOR),
                 ...
                 compressor_(nullptr),
                 is_inited_(false) {}
  int init(common::TSDataType data_type,
  ...
  FORCE_INLINE int write(int64_t timestamp, bool    value) { PW_DO_WRITE_FOR_TYPE(common::BOOLEAN); }
  FORCE_INLINE int write(int64_t timestamp, int32_t value) { PW_DO_WRITE_FOR_TYPE(common::INT32);   }
  FORCE_INLINE int write(int64_t timestamp, int64_t value) { PW_DO_WRITE_FOR_TYPE(common::INT64);   }
  FORCE_INLINE int write(int64_t timestamp, float   value) { PW_DO_WRITE_FOR_TYPE(common::FLOAT);   }
  FORCE_INLINE int write(int64_t timestamp, double  value) { PW_DO_WRITE_FOR_TYPE(common::DOUBLE);  }
  ...
  int write_to_chunk(common::ByteStream &pages_data,
                     bool write_header,
                     bool write_statistic,
                     bool write_data_to_chunk_data);
  FORCE_INLINE common::ByteStream &get_time_data() { return time_out_stream_; }
  FORCE_INLINE common::ByteStream &get_value_data() { return value_out_stream_; }
...
private:
  ...
  int copy_page_data_to(common::ByteStream &my_page_data, common::ByteStream &pages_data);
...
#endif // STORAGE_TSFILE_PAGE_WRITER_H
