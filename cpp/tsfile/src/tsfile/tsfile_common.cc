...
/* ================ TimeseriesIndex ================ */
int TimeseriesIndex::add_chunk_meta(ChunkMeta *chunk_meta, bool serialize_statistic)
{
  int ret = E_OK;
  if (IS_NULL(chunk_meta)) {
    ret = E_INVALID_ARG;
  } else if (RET_FAIL(chunk_meta->serialize_to(chunk_meta_list_serialized_buf_, serialize_statistic))) {
...
int TSMIterator::init()
{
   ...
   * do sort now.
   * Currently, MemTableFlusher guarantee that chunk_group_meta_list_ is sorted
   */
  // do nothing.
  return E_OK;
}
...
int TSMIterator::get_next(String &ret_device_name,
                          String &ret_measurement_name,
                          TimeseriesIndex &ret_ts_index)
{
  int ret = E_OK;
  SimpleList<ChunkMeta*> chunk_meta_list_of_this_ts(1024, MOD_TIMESERIES_INDEX_OBJ); // FIXME
  String cur_measurement_name;

  if (chunk_meta_iter_ != chunk_group_meta_iter_.get()->chunk_meta_list_.end()) {
    cur_measurement_name.shallow_copy_from(chunk_meta_iter_.get()->measurement_name_);
  } else {
  ...
  while (chunk_meta_iter_ != chunk_group_meta_iter_.get()->chunk_meta_list_.end()) {
    if (cur_measurement_name.equal_to(chunk_meta_iter_.get()->measurement_name_)) {
      if (RET_FAIL(chunk_meta_list_of_this_ts.push_back(chunk_meta_iter_.get()))) {
      } else {
        chunk_meta_iter_++;
      }
    } else {
  ...
  if (chunk_meta_list_of_this_ts.size() == 0) {
    log_error("fatal error, empty chunk meta list for %s", cur_measurement_name.buf_);
  ...
  SimpleList<ChunkMeta*>::Iterator ts_chunk_meta_iter = chunk_meta_list_of_this_ts.begin();
  for (; IS_SUCC(ret) && ts_chunk_meta_iter != chunk_meta_list_of_this_ts.end();
         ts_chunk_meta_iter++) {
  ...
  if (UNLIKELY(ret_device_name.is_null())) {
    ...
    log_error("null device name from chunk_group_meta_iter, ret=%d", ret);
    ASSERT(false);
  }
  return ret;
}
...
{
#if DEBUG_SE
  std::cout << "MetaIndexNode::binary_search_children start, name=" << name << ", exact_search=" << exact_search << ", children_.size=" << children_.size() << std::endl;
...
#if DEBUG_SE
    std::cout << "MetaIndexNode::binary_search_children doing, cmp: cur=" << children_[m]->name_ << ", name=" << name << ", exact_search=" << exact_search << ", children_.size=" << children_.size() << std::endl;
#endif
    if (cmp == 0) {
      l = m;
      found = true;
...
#if DEBUG_SE
  std::cout << "MetaIndexNode::binary_search_children end, ret=E_NOT_EXIST, name=" << name << ", exact_search=" << exact_search << std::endl;
#endif
    return E_NOT_EXIST;
  }
...
#if DEBUG_SE
  std::cout << "MetaIndexNode::binary_search_children end, ret_index_entry=" << ret_index_entry << ", ret_end_offset=" << ret_end_offset << ", name=" << name << ", exact_search=" << exact_search << std::endl;
#endif
  return E_OK;
}
