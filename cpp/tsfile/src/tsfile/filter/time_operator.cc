...
{
  if (not_) {
    return statistic->end_time_ < value1_ || statistic->start_time_ > value2_;
  } else {
    return statistic->end_time_ >= value1_ && statistic->start_time_ <= value2_;
  }
}
...
std::vector<TimeRange*>* TimeBetween::get_time_ranges()
{
  ...
  if (not_) {
    if (value1_ != std::numeric_limits<int64_t>::min()) {
      result->push_back(new TimeRange(std::numeric_limits<int64_t>::min(), value1_ - 1));
    }
    if (value2_ != std::numeric_limits<int64_t>::max()) {
      result->push_back(new TimeRange(value2_ + 1, std::numeric_limits<int64_t>::max()));
    }
  } else {
...
{
  std::vector<int64_t>::iterator it = find(values_.begin(), values_.end(), time);
  bool result = (it != values_.end() ? true : false);
  return result != not_;
}

bool TimeIn::satisfy_start_end_time(long start_time, long end_time) { return true; }

bool TimeIn::contain_start_end_time(long start_time, long end_time) { return true; }
...
bool TimeEq::satisfy(long time, int64_t value)
{
  return value_ == time;
}
...
bool TimeNotEq::satisfy(long time, int64_t value)
{
  return !(value_ == time);
}
...
std::vector<TimeRange*>* TimeNotEq::get_time_ranges()
{
  ...
  if (value_ == std::numeric_limits<int64_t>::min()) {
    result->push_back(new TimeRange(value_ + 1, std::numeric_limits<int64_t>::max()));
  } else if (value_ == std::numeric_limits<int64_t>::max()) {
    result->push_back(new TimeRange(std::numeric_limits<int64_t>::min(), value_ - 1));
  } else {
    result->push_back(new TimeRange(std::numeric_limits<int64_t>::min(), value_ - 1));
    result->push_back(new TimeRange(value_ + 1, std::numeric_limits<int64_t>::max()));
  }
...
bool TimeGt::satisfy(long time, int64_t value)
{
  return value_ < time;
}
...
bool TimeGtEq::satisfy(long time, int64_t value)
{
  return value_ <= time;
}
...
{
  std::vector<TimeRange*> *result = new std::vector<TimeRange*>();
  result->push_back(new TimeRange(value_, std::numeric_limits<int64_t>::max()));
  return result;
}
...
bool TimeLt::satisfy(long time, int64_t value)
{
  return value_ > time;
}
...
bool TimeLtEq::satisfy(long time, int64_t value)
{
  return value_ >= time;
}
...
{
  std::vector<TimeRange*> *result = new std::vector<TimeRange*>();
  result->push_back(new TimeRange(std::numeric_limits<int64_t>::min(), value_));
  return result;
}
