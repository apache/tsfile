...
#ifndef STORAGE_TSFILE_BLOOM_FILTER_H
#define STORAGE_TSFILE_BLOOM_FILTER_H
...
public:
  ...
  int init(int32_t size)
  {
    ...
    words_ = (uint64_t*)common::mem_alloc(alloc_size, common::MOD_BLOOM_FILTER);
    if (IS_NULL(words_)) {
      return common::E_OOM;
    }
...
private:
  common::String get_entry_string(const common::String &device_name,
                                  const common::String &measurement_name);
  FORCE_INLINE void free_entry_buf(char *entry_buf) { common::mem_free(entry_buf); }
private:
  uint32_t size_;
