...
#ifndef STORAGE_TSFILE_CHUNK_READER_H
#define STORAGE_TSFILE_CHUNK_READER_H

#include "common/allocator/my_string.h"
#include "tsfile/tsfile_common.h"
#include "tsfile/read_file.h"
#include "tsfile/encode/decoder.h"
...
{
public:
  ChunkReader() : read_file_(nullptr),
                  chunk_meta_(nullptr),
                  measurement_name_(),
                  chunk_header_(),
                  cur_page_header_(),
  ...
  bool has_more_data() const
  {
    return prev_page_not_finish()
           || (chunk_visit_offset_ - chunk_header_.serialized_size_ < chunk_header_.data_size_);
  }
...
private:
  ...
  int alloc_compressor_and_value_decoder(common::TSEncoding encoding,
                                         common::TSDataType data_type,
  ...
  int skip_cur_page();
  int decode_cur_page_data(common::TsBlock *&ret_tsblock,
                           Filter *filter);
  int decode_tv_buf_into_tsblock(char *time_buf, 
                                 char *value_buf,
  ...
  {
    return (time_decoder_ && time_decoder_->has_remaining()) || time_in_.has_remaining();
  }
...
private:
  ...
  /*
   * Data read from file is stored in @in_stream_, and the size
...
#endif // STORAGE_TSFILE_CHUNK_READER_H
