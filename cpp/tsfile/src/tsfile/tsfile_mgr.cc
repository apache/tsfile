...
int TsFileMgr::add_new_file(const FileID &file_id,
                            OpenFile *open_file)
{
  MutexGuard mg(all_open_files_mutex_);
                                   ...
                                   int64_t &ret_version)
{
  ...
  for (AllOpenFileMapIter iter = all_open_files_.begin();
       iter != all_open_files_.end() && IS_SUCC(ret);
       iter++) {
    ...
    if (tmp_ret == E_OK) {
      ...
      ret = tmp_ret;
      log_error("get time range for ts_id error, ret=%d, ts_id=%s", ret, ts_id.to_string().c_str());
    }
  } // end for
  ...
  std::sort(tsfile_vec.begin(), tsfile_vec.end(), compare_timerange_openfile_pair);
