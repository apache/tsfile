...
#include "tsfile/filter/and_filter.h"
#include "tsfile/filter/or_filter.h"
...
void QueryExpression::add_time_filter_to_series_filter(Filter *time_filter,
...
void QueryExpression::add_time_filter_to_query_filter(Filter *time_filter, Expression *expression)
{
  if (expression->type_ == SERIES_EXPR) {
    add_time_filter_to_series_filter(time_filter, expression);
  } else if ((expression->type_ == AND_EXPR) || (expression->type_ == OR_EXPR)) {
    ...
    std::cout << "Expression should contains only SingleSeriesExpression but other type is found" << std::endl;
  }
}

Expression* QueryExpression::combine_two_global_time_filter(Expression *left,
                                                            Expression *right,
                                                            ExpressionType type)
{
  if (type == AND_EXPR) {
...
bool QueryExpression::update_filter_with_or(Expression *expression, Filter *filter, Path &path)
{
  if (expression->type_ == SERIES_EXPR && expression->series_path_ == path) {
    ...
    return update_filter_with_or(left, filter, path) || update_filter_with_or(right, filter, path);
  } else {
    return false;
  }
}

Expression* QueryExpression::merge_second_tree_to_first_tree(Expression *left_expression,
    Expression *right_expression)
{
  if (right_expression->type_ == SERIES_EXPR) {
    Expression *leaf = right_expression;
    update_filter_with_or(left_expression, leaf->filter_, leaf->series_path_);
    ...
    left_expression = merge_second_tree_to_first_tree(left_expression, left_child);
    left_expression = merge_second_tree_to_first_tree(left_expression, right_child);
    return left_expression;
  } else {
    Expression *expr = new Expression(OR_EXPR, left_expression, right_expression);
    my_exprs_.push_back(expr);
    return expr;
  }
}

Expression* QueryExpression::push_global_time_filter_to_all_series(Expression *time_filter,
                                                                   std::vector<Path> &selected_series)
{
  ...
  Expression *expression = new Expression(SERIES_EXPR, selected_series.at(0), time_filter->filter_);
  my_exprs_.push_back(expression);
  for (uint32_t i = 1; i < selected_series.size(); i++) {
    Expression *r = new Expression(SERIES_EXPR, selected_series.at(i), time_filter->filter_);
...
Expression* QueryExpression::handle_one_global_time_filter(Expression *left,
    Expression *expression, std::vector<Path> &selected_series, ExpressionType type)
{
  ...
  if (type == AND_EXPR) {
    ...
    Expression *after_transform = push_global_time_filter_to_all_series(left, selected_series);
...
Expression* QueryExpression::optimize(Expression *expression, std::vector<Path> &series_paths)
{
  ...
  if (type == GLOBALTIME_EXPR || type == SERIES_EXPR) {
    ...
    if (left->type_ == GLOBALTIME_EXPR && right->type_ == GLOBALTIME_EXPR) {
    ...
    } else if (left->type_ == GLOBALTIME_EXPR && right->type_ != GLOBALTIME_EXPR) {
      return handle_one_global_time_filter(left, right, series_paths, type);
    } else if (left->type_ != GLOBALTIME_EXPR && right->type_ == GLOBALTIME_EXPR) {
      return handle_one_global_time_filter(right, left, series_paths, type);
    } else if (left->type_ != GLOBALTIME_EXPR && right->type_ != GLOBALTIME_EXPR) {
      ...
      if (type == AND_EXPR) {
        ...
        std::cout << "unsupported Expression type:" << type << std::endl;
      }

      if (mid_ret->left_->type_ == GLOBALTIME_EXPR || mid_ret->right_->type_ == GLOBALTIME_EXPR) {
