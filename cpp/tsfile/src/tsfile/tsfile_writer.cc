...
using namespace timecho::storage;
...
typedef std::map<std::string, MeasurementSchemaGroup*>::iterator DeviceSchemaIter;
...
TsFileWriter::TsFileWriter() : write_file_(nullptr),
                               io_writer_(nullptr),
                               schemas_(),
...
void TsFileWriter::destroy()
{
  ...
  for (dev_iter = schemas_.begin(); dev_iter != schemas_.end(); dev_iter++) {
    MeasurementSchemaMap &ms_map = dev_iter->second->measurement_schema_map_;
...
{
  MeasurementSchema *ms = new MeasurementSchema(measurement_name,
                                                data_type,
                                                encoding,
                                                compression_type);
...
{
  int ret = E_OK;
  std::vector<MeasurementSchema*>::const_iterator it = measurement_schema_vec.begin();
  for (; it != measurement_schema_vec.end(); it++) { // cppcheck-suppress postfixOperator
...
struct MeasurementNamesFromRecord
{
  ...
  explicit MeasurementNamesFromRecord(const TsRecord &record) : record_(record) {}
...
struct MeasurementNamesFromTablet
{
  ...
  explicit MeasurementNamesFromTablet(const Tablet &tablet) : tablet_(tablet) {}
  FORCE_INLINE uint32_t get_count() const { return tablet_.schema_vec_->size(); }
  FORCE_INLINE const std::string& at(size_t idx) const
  {
                                  ...
                                  // std::vector<ChunkWriter*> &chunk_writers)
{
  ...
  for (uint32_t i = 0; i < measurement_count; i++) {
    ...
    if (UNLIKELY(ms_iter == msm.end())) {
      ...
      // Here we may check data_type against ms_iter. But in Java libtsfile, no check here.
      MeasurementSchema *ms = ms_iter->second;
...
int TsFileWriter::write_record(const TsRecord &record)
{
  ...
  if (RET_FAIL(do_check_schema(record.device_name_, mnames_getter, chunk_writers))) {
    return ret;
  }
  ...
  for (uint32_t c = 0; c < chunk_writers.size(); c++) {
    ...
    // ignore point write failure
    write_point(chunk_writer, record.timestamp_, record.points_[c]);
  }
  return ret;
}

int TsFileWriter::write_point(ChunkWriter *chunk_writer,
                              int64_t timestamp,
...
int TsFileWriter::write_tablet(const Tablet &tablet)
{
  ...
  if (RET_FAIL(do_check_schema(tablet.device_name_, mnames_getter, chunk_writers))) {
    return ret;
  }
...
int TsFileWriter::write_column(ChunkWriter *chunk_writer, const Tablet &tablet, int col_idx)
{
  int ret = E_OK;
  ...
  if (data_type == timecho::common::BOOLEAN) {
    ret = write_typed_column(chunk_writer, timestamps, (bool*)col_values, col_bitmap, row_count);
  } else if (data_type == timecho::common::INT32) {
    ret = write_typed_column(chunk_writer, timestamps, (int32_t*)col_values, col_bitmap, row_count);
  } else if (data_type == timecho::common::INT64) {
    ret = write_typed_column(chunk_writer, timestamps, (int64_t*)col_values, col_bitmap, row_count);
  } else if (data_type == timecho::common::FLOAT) {
    ret = write_typed_column(chunk_writer, timestamps, (float*)col_values, col_bitmap, row_count);
  } else if (data_type == timecho::common::DOUBLE) {
    ret = write_typed_column(chunk_writer, timestamps, (double*)col_values, col_bitmap, row_count);
  } else {
    ASSERT(false);
  }
  return ret;
...
int TsFileWriter::flush()
{
  ...
  for (device_iter = schemas_.begin(); device_iter != schemas_.end(); device_iter++) { // cppcheck-suppress postfixOperator
...
int TsFileWriter::flush_chunk_group(MeasurementSchemaGroup *chunk_group)
{
  ...
  for (MeasurementSchemaMapIter ms_iter = map.begin(); ms_iter != map.end(); ms_iter++) {
    ...
    } else if (RET_FAIL(io_writer_->start_flush_chunk(chunk_writer->get_chunk_data(),
            m_schema->measurement_name_, m_schema->data_type_, m_schema->encoding_,
            m_schema->compression_type_, chunk_writer->num_of_pages()))) {
    } else if (RET_FAIL(io_writer_->flush_chunk(chunk_writer->get_chunk_data()))) {
    } else if (RET_FAIL(io_writer_->end_flush_chunk(chunk_writer->get_chunk_statistic()))) {
    }
  }
  return ret;
}
...
{
    std::cout<<"will write close magic code"<<std::endl;
