                                  ...
                                  QueryExpression *qe)
{
  ...
  for (size_t i = 0; i < origin_path_count; i++) {
    ...
    ret = io_reader_->alloc_ssi(paths[i].device_,
            paths[i].measurement_, ssi, global_time_filter);
    if (ret != 0) {
        return ret;
    } else {
...
RowRecord *QDSWithoutTimeGenerator::get_next()
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    uint32_t len = 0;
    row_record_->get_field(iter->second)->set_value(
        value_iters_[iter->second]->get_data_type(),
