...
ErrCode write_tsfile() {
    ErrCode err_code;
    CTsFileWriter writer;
    if (access("test.ts", 0) == 0) {
        if (remove("test.ts") != 0) {
            printf("Failed to delete test.ts file\n");
            return -1;
        }
    }
    writer = ts_writer_open("test.ts", &err_code);
    if (NULL == writer) {
        return err_code;
    }
...
ErrCode read_tsfile() {
    ...
    reader = ts_reader_open("test.ts", &err_code);
    if (NULL == reader) {
        return err_code;
    }
