...
class IntFire : public Fire<int> {
   public:
    ...
    int predict(int value) override {
        ...
        int diff = (alpha * delta_) >> bit_width_;
        return value + diff;
    }
...
class LongFire : public Fire<long long> {
   public:
    ...
    long long predict(long long value) override {
        long long alpha = accumulator_ >> learn_shift_;
        long long diff = (alpha * delta_) >> bit_width_;
        return value + diff;
    }

    void train(long long pre, long long val, long long err) override {
        long long gradient = err > 0 ? -delta_ : delta_;
        accumulator_ -= gradient;
        delta_ = val - pre;
    }
};
