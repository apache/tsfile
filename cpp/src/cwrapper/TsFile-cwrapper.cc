...
Tablet* add_column_to_tablet(Tablet* tablet, char* column_name,
    ...
    // tablet->bitmap = (bool**)realloc(tablet->bitmap, tablet->column_num *
    // sizeof(bool*)); tablet->bitmap[tablet->column_num - 1] =
    // (bool*)malloc((tablet->max_capacity/8 +1) * sizeof(bool));
...
Tablet* add_data_to_tablet(Tablet* tablet, int line_id, int64_t timestamp,
    ...
    memcpy((int64_t*)tablet->value[column_id] + line_id, &value,
           sizeof(int64_t));
    // int map_id = line_id / 8;
    // int bit_id = line_id % 8;
    // tablet->bitmap[column_id][map_id] |= 1 << bit_id;
        ...
        for (int col = 0; col < column_num; col++) {
            ...
            switch (field->type_) {
                ...
                case TSDataType::INT64:
                    ...
                    break;
                case TSDataType::FLOAT:
                    f = field->value_.fval_;
                    memcpy(&datum, &f, sizeof(float));
                    result =
                        add_data_to_tablet(result, i, record->get_timestamp(),
                                           data->column_names[col], datum);
                    break;
                case TSDataType::DOUBLE:
                    d = field->value_.dval_;
                    memcpy(&datum, &d, sizeof(double));
                    result =
                        add_data_to_tablet(result, i, record->get_timestamp(),
                                           data->column_names[col], datum);
                    break;
                case TSDataType::TEXT:
                    text = field->value_.sval_;
                    datum = reinterpret_cast<int64_t>(text);
                    result =
                        add_data_to_tablet(result, i, record->get_timestamp(),
                                           data->column_names[col], datum);
                    break;
