...
struct TsID {
  ...
  TsID & operator = (const TsID &other) 
  {
    ...
    FORCE_INLINE void to_string(char *print_buf, int len) const {
        snprintf(print_buf, static_cast<size_t>(len), "<%d,%d,%d>", db_nid_, device_nid_,
                 measurement_nid_);
    }
};
...
struct ColumnSchema {
    ...
    std::string get_device_name_str() const {
        size_t pos = column_name_.find_last_of('.');
        ASSERT(pos > 0);
    ...
    void get_device_name(String &device_name) const {
        ...
        device_name.len_ = static_cast<int32_t>(pos);
    }
