   ...
   public:
    FORCE_INLINE static int32_t hash(int32_t value, uint32_t seed) {
        return static_cast<int32_t>(
            inner_hash(reinterpret_cast<const char *>(&value), 4, seed));
    }

    FORCE_INLINE static int32_t hash(int64_t value, uint32_t seed) {
        return static_cast<int32_t>(
            inner_hash(reinterpret_cast<const char *>(&value), 8, seed));
    }
    ...
    FORCE_INLINE static int32_t hash(std::string &value, uint32_t seed) {
        return static_cast<int32_t>(inner_hash(
            value.data(), static_cast<int32_t>(value.size()), seed));
    }

    FORCE_INLINE static int32_t hash(const common::String &buf, int32_t seed) {
        return static_cast<int32_t>(inner_hash(buf.buf_, buf.len_ - 1, seed));
    }

   private:
    FORCE_INLINE static int64_t rotl64(int64_t v, int n) {
        auto uv = static_cast<uint64_t>(v);
        return static_cast<int64_t>((uv << n) | (uv >> (64 - n)));
    }
