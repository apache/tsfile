...
int compare_string_views(const DatasetIndexStringView& left,
                         const DatasetIndexStringView& right) {
    const uint32_t common_length = std::min(left.length, right.length);
    if (common_length != 0) {
        const int comparison =
            std::memcmp(left.data, right.data, common_length);
        if (comparison != 0) {
            return comparison;
        }
    }
...
DatasetIndexStatus DatasetIndexWriter::write_atomic(
    ...
    if (input_sections.size() != DATASET_INDEX_SECTION_COUNT) {
        error_message = "current v1 writer requires exactly 13 sections";
        return DatasetIndexStatus::INVALID_ARGUMENT;
    }
...
#undef DATASET_RECORDS
    ...
    if (table_names.count != tables.count) {
        return fail(DatasetIndexStatus::BAD_REFERENCE,
                    "TableNameIndex must contain one entry per TableRecord");
    }
    std::vector<uint8_t> seen_table_ids(tables.count, 0);
    DatasetIndexStringView previous_table_name;
    uint64_t previous_table_hash = 0;
    bool have_previous_table_name = false;
    for (uint32_t i = 0; i < table_names.count; ++i) {
        const TableNameIndexRecord& value = table_name_records[i];
        if (value.name_sid >= string_count || value.table_id >= tables.count ||
            seen_table_ids[value.table_id] != 0 ||
            table_records[value.table_id].name_sid != value.name_sid) {
            return fail(
                DatasetIndexStatus::BAD_REFERENCE,
                "TableNameIndex contains an invalid or duplicate reference");
        }
        DatasetIndexStringView table_name;
        if (string(value.name_sid, table_name) != DatasetIndexStatus::OK ||
            dataset_index_name_hash(table_name.data, table_name.length) !=
                value.name_hash) {
            return fail(DatasetIndexStatus::BAD_REFERENCE,
                        "TableNameIndex name hash does not match its string");
        }
        if (have_previous_table_name &&
            (value.name_hash < previous_table_hash ||
             (value.name_hash == previous_table_hash &&
              compare_string_views(table_name, previous_table_name) <= 0))) {
            return fail(
                DatasetIndexStatus::BAD_REFERENCE,
                "TableNameIndex is unsorted or has duplicate table names");
        }
        seen_table_ids[value.table_id] = 1;
        previous_table_name = table_name;
        previous_table_hash = value.name_hash;
        have_previous_table_name = true;
    }
    ...
    for (uint32_t i = 0; i < files.count; ++i) {
        ...
        if (value.path_sid >= string_count || value.reserved0 != 0 ||
            value.reserved1 != 0) {
            return fail(DatasetIndexStatus::BAD_REFERENCE,
                        "TsFileRecord contains an invalid reference");
        }
    }
        ...
        if (value.device_id >= devices.count || value.file_id >= files.count ||
            ...
            (value.layout == 0 &&
             (value.time_meta_offset != 0 || value.time_meta_length != 0 ||
              value.row_count != 0)) ||
            (value.layout == 1 &&
             (value.time_meta_length == 0 || value.row_count == 0))) {
            return fail(
                DatasetIndexStatus::BAD_REFERENCE,
    ...
    for (uint32_t i = 0; i < locators.count; ++i) {
        ...
        if (value.device_file_span_id >= device_spans.count ||
            value.locator_kind > 1 || value.flags != 0 || value.padding != 0 ||
