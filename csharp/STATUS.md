...
**Status**: Production Ready (v3 format), v4 format NOT supported

⚠️ **CRITICAL LIMITATION**: This C# implementation currently only supports TSFile **v3 format**.
Java TSFile library generates **v4 format** files, which are **NOT compatible** with this C# implementation.
For production use with Java interoperability, Java must be configured to write v3 format files.

## Executive Summary

The C# implementation of Apache TSFile provides a production-ready time-series file format library for **v3 format files** with:
...
- ⚠️ **Binary format compatibility LIMITED to v3 only**
...
| Version | C# Support | Java Default | Status |
|---------|------------|--------------|--------|
| v3 | ✅ Full support | No (uses v4) | C# writes v3, can read v3 |
| v4 | ❌ Not supported | ✅ Yes | Java writes v4 by default |

### Critical Limitation: v4 Format Not Supported

**The C# implementation cannot read TSFile v4 format files generated by Java.**

**Why v4 is not supported:**
- v4 introduces a complex metadata structure with recursive `MetadataIndexNode` trees
- The metadata layout is fundamentally different from v3:
  - v3: `[metadata_offset: 8 bytes][MAGIC: 6 bytes]`
  - v4: `[TsFileMetadata_size: 4 bytes][MAGIC: 6 bytes]` with metadata_offset stored inside TsFileMetadata
- v4's `MetadataIndexNode` uses a hierarchical index structure for efficient querying
- Full v4 support requires implementing complex tree deserialization with proper recursion handling

**Impact:**
- ❌ C# **cannot** read Java-generated TSFile files (Java uses v4 by default)
- ❌ Java-C# interoperability is **broken** without configuration changes
- ❌ The 360 interop test files cannot be validated
- ✅ C# can still write and read its own v3 files
- ✅ C# to C# interoperability works

**Workaround:**
To achieve Java-C# interoperability, Java must be configured to write v3 format files.
(Configuration method needs to be documented in Java TSFile library)

**To Fix:**
Implement full v4 format support in C# TsFileReader:
1. Implement `MetadataIndexNode` deserialization with proper tree recursion
2. Implement v4 `TsFileMetadata` deserialization
3. Handle v4-specific features (table-based indexing, etc.)
4. Add comprehensive v4 format tests
