...
RECORDS: Mapping[int, struct.Struct] = {
    ...
    TABLE_RECORD: struct.Struct("<IIQIIIIIIQ"),
    ...
    TSFILE_RECORD: struct.Struct("<IIQqqQQ"),
    TABLE_FILE_SPAN: struct.Struct("<IIqqQ"),
    DEVICE_FILE_SPAN: struct.Struct("<IIQIHHqqQ"),
    SERIES_FILE_SPAN: struct.Struct("<IIIIqqqQ"),
...
def write_index_atomic(path: str, section_payloads: Mapping[int, bytes]) -> None:
    ...
    if set(section_payloads) != set(range(1, SECTION_COUNT + 1)):
        raise ValueError("Dataset Index v1 requires exactly 14 sections")
...
class MappedDatasetIndex:
    ...
    def find_device_id(self, table_id: int, name: str) -> int:
        ...
        return self._find_child(DEVICE_NAME_INDEX, table_id, name, table[3], table[4])

    def find_column_id(self, table_id: int, name: str) -> int:
        table = self.record(TABLE_RECORD, table_id)
        return self._find_child(COLUMN_NAME_INDEX, table_id, name, table[5], table[6])
    ...
    try:
        with MappedDatasetIndex(path) as index:
            ...
            for file_id in range(index.count(TSFILE_RECORD)):
                ...
                if (
                    st.st_size != record[2]
                    or file_fingerprint(file_path, st) != record[5]
                ):
                    return False
...
def _schema_fingerprint(table_entry, field_types: Mapping[str, int]) -> int:
    parts = []
    for name, data_type in zip(table_entry.tag_columns, table_entry.tag_types):
        parts.append(f"T\0{name}\0{int(data_type)}\01")
    for name in table_entry.field_columns:
        parts.append(f"F\0{name}\0{field_types.get(name, -1)}\01")
    return name_hash("\0".join(parts).encode("utf-8"))
...
def build_sections_from_dataframe(dataframe) -> Mapping[int, bytes]:
    ...
    for table_name in table_names:
        ...
        definitions = [
            (name, int(data_type), 0)
            for name, data_type in zip(table.tag_columns, table.tag_types)
        ] + [
            (name, field_types.get((table_name, name), -1), 1)
            for name in table.field_columns
        ]
        for ordinal, (name, data_type, role) in enumerate(definitions):
    ...
    series_span_rows = []
    device_span_specs = {}
    file_bounds = {file_id: [None, None] for file_id in range(len(file_paths))}
    series_spans_by_id = defaultdict(list)
    device_spans_by_device = defaultdict(list)
    table_spans = defaultdict(lambda: [None, None, 0])
    ...
    for _, _, old_series_ref in series_specs:
        ...
        for reader, local_device, local_field in index.series_shards[old_series_ref]:
            ...
            if previous is None:
                ...
                device_span_specs[span_key] = (
                    *previous[:4],
                    min(previous[4], candidate[4]),
                    max(previous[5], candidate[5]),
                    max(previous[6], candidate[6]),
                )
            ...
            for bounds in (file_bounds[file_id], table_spans[(table_id, file_id)]):
                bounds[0] = (
                    stats.timeline_min_time
                    if bounds[0] is None
                    else min(bounds[0], stats.timeline_min_time)
                )
                bounds[1] = (
                    stats.timeline_max_time
                    if bounds[1] is None
                    else max(bounds[1], stats.timeline_max_time)
                )
    ...
    for key, values in sorted(device_span_specs.items()):
        ...
        table_id = device_specs[device_id][0]
        table_spans[(table_id, file_id)][2] += values[6]
    locator_rows = [
        (device_span_ids[key], kind, flags, offset, length, count)
        for key, kind, flags, offset, length, count in locators
    ]
    ...
    for series_id, (device_id, column_id, _) in enumerate(series_specs):
        ...
        prefix_max = None
        for row in spans:
            prefix_max = row[5] if prefix_max is None else max(prefix_max, row[5])
            row[6] = prefix_max
    ...
    table_span_rows = []
    table_spans_by_table = defaultdict(list)
    for (table_id, file_id), (minimum, maximum, rows) in sorted(
        table_spans.items(), key=lambda item: (item[0][0], item[1][0], item[0][1])
    ):
        span_id = len(table_span_rows)
        table_span_rows.append((table_id, file_id, minimum, maximum, rows))
        table_spans_by_table[table_id].append(span_id)
    ...
    for table_name in table_names:
        ...
        spans = table_spans_by_table[table_id]
        first_span = spans[0] if spans else 0
        field_type_map = {
            name: field_types.get((table_name, name), -1)
            for name in table.field_columns
        }
        table_rows.append(
            (
                name_sid,
                0,
                _schema_fingerprint(table, field_type_map),
                first_device,
                len(names),
                first_column,
                len(column_names),
    ...
    for file_id, path in enumerate(file_paths):
        st = os.stat(path)
        minimum, maximum = file_bounds[file_id]
        if minimum is None:
            minimum = maximum = 0
        file_rows.append(
            (
    ...
    return {
        ...
        TABLE_FILE_SPAN: _pack_records(TABLE_FILE_SPAN, table_span_rows),
