...
*/
public abstract class FloatFilter extends ValueFilter {
  ...
  protected abstract boolean valueSatisfy(float value);
    ...
    for (int i = 0; i < selection.length; i++) {
      if (selection[i]) {
        if (valueColumn.isNull(i)) {
          ...
          satisfyInfo[i] = valueSatisfy(valueColumn.getFloat(i));
        }
      }
    }
    return satisfyInfo;
