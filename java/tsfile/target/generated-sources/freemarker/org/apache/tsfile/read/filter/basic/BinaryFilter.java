...
*/
public abstract class BinaryFilter extends ValueFilter {

  protected BinaryFilter(int measurementIndex) {
    super(measurementIndex);
  }

  protected BinaryFilter(ByteBuffer buffer) {
    super(buffer);
  }
  ...
  protected abstract boolean valueSatisfy(Binary value);
    ...
    for (int i = 0; i < selection.length; i++) {
      if (selection[i]) {
        if (valueColumn.isNull(i)) {
          ...
          satisfyInfo[i] = valueSatisfy(valueColumn.getBinary(i));
        }
      }
    }
    return satisfyInfo;
