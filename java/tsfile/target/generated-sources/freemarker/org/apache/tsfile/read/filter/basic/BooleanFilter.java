...
*/
public abstract class BooleanFilter extends ValueFilter {

  protected BooleanFilter(int measurementIndex) {
    super(measurementIndex);
  }

  protected BooleanFilter(ByteBuffer buffer) {
    super(buffer);
  }
  ...
  protected abstract boolean valueSatisfy(boolean value);
    ...
    for (int i = 0; i < selection.length; i++) {
      if (selection[i]) {
        if (valueColumn.isNull(i)) {
          ...
          satisfyInfo[i] = valueSatisfy(valueColumn.getBoolean(i));
        }
      }
    }
    return satisfyInfo;
