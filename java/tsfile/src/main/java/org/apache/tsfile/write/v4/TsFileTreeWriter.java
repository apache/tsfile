...
/**
 * TsFileTreeWriter provides a tree-based interface for writing data to TsFiles.
 * It simplifies the writing process by handling schema registration and data writing
 * with a focus on tree model operations.
 */
public class TsFileTreeWriter implements AutoCloseable {
     ...
     */
    public void registerTimeseries(String deviceId, IMeasurementSchema schema) throws WriteProcessException {
        try {
     ...
     * @throws WriteProcessException if registration fails
     */
    public void registerAlignedTimeseries(String deviceId, List<IMeasurementSchema> schemas) throws WriteProcessException {
        try {
            tsFileWriter.registerAlignedTimeseries(deviceId, schemas);
        } catch (WriteProcessException e) {
            throw new WriteProcessException("Failed to register aligned timeseries for device " + deviceId, e);
        }
    }
