...
public class RecordUtils {
  ...
  public static TSRecord parseSimpleTupleRecord(String str, Schema schema) {
    ...
    try {
      ...
      LOG.warn(Messages.get("log.utils.record_timestamp_illegal"), str);
    ...
    for (int i = 2; i < items.length - 1; i += 2) {
      ...
      if (measurementSchema == null) {
        LOG.warn(Messages.get("log.utils.record_measurement_type_not_found"), measurementId);
        continue;
      }
      ...
      if (!"".equals(value)) {
        try {
          ret.addTuple(Type.fromTsDataType(type).getDataPoint(measurementId, value));
        } catch (NumberFormatException e) {
          LOG.warn(Messages.get("log.utils.record_parse_measurement_error"), e);
        } catch (UnSupportedDataTypeException e) {
          LOG.warn(Messages.get("log.utils.record_unsupported_data_type"), type);
        }
      }
    }
    return ret;
