...
public class ALPTest {
  ...
  public static int BlockEncoderOptimalPackV5(
      double[] data,
      int block_index,
      int block_size,
      int remainder,
    ...
    int packSize = OptimalPackSize.findOptimalPackSizeallV5(asLong);
    ...
    byte[] body = OptimalPackSize.encodeBitPackingV2(asLong, bitWidths, packSize);
    int need = encode_pos + 1 + 4 + 4 + body.length;
    if (need > encoded_result.length) {
      throw new IllegalStateException(
          "encoded_result too small: need " + need + " have " + encoded_result.length);
    }
  ...
  public static int BlockDecoderOptimalPackV5(
      byte[] encoded_result,
      int block_index,
      int block_size,
      int remainder,
    ...
    if (submodeAfterMin) {
      ...
      if (mode == ALP_V5_SUBMODE_LONG_PACK8) {
        ...
        encode_pos += bodyLen;
        long[] decoded = OptimalPackSize.decodeBitPackingV2Auto(body, packSize, remainder);
        for (int i = 0; i < remainder; i++) {
  ...
  public static int EncoderOptimalPackV5(
      double[] data, int block_size, int max_decimal, byte[] encoded_result) {
    ...
    for (int i = 0; i < num_blocks; i++) {
      encode_pos =
          BlockEncoderOptimalPackV5(
              data, i, block_size, block_size, encode_pos, max_decimal, encoded_result);
    }
    if (remainder > 0) {
      encode_pos =
          BlockEncoderOptimalPackV5(
              data, num_blocks, block_size, remainder, encode_pos, max_decimal, encoded_result);
    }
    return encode_pos;
  }
  ...
  public static double[] DecoderOptimalPackV5(byte[] encoded_result) {
    ...
    for (int i = 0; i < num_blocks; i++) {
      encode_pos =
          BlockDecoderOptimalPackV5(
              encoded_result, i, block_size, block_size, encode_pos, data, submodeAfterMin);
    }
    if (remainder > 0) {
      encode_pos =
          BlockDecoderOptimalPackV5(
              encoded_result, num_blocks, block_size, remainder, encode_pos, data, submodeAfterMin);
    }
    return data;
  }
  ...
  public static int Encoder(double[] data, int block_size, int max_decimal, byte[] encoded_result) {
    ...
    for (int i = 0; i < num_blocks; i++) {
      encode_pos =
          BlockEncoder(data, i, block_size, block_size, encode_pos, max_decimal, encoded_result);
    }
  ...
  public static double[] Decoder(byte[] encoded_result) {
    ...
    if (remainder > 0) {
      encode_pos =
          BlockDecoder(encoded_result, num_blocks, block_size, remainder, encode_pos, data);
    }
