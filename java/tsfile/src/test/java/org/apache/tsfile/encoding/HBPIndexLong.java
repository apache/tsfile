...
public class HBPIndexLong {
  ...
  public HBPIndexLong(int kBits, long[] codes) {
    ...
    if (sectionsPerWord <= 0) throw new IllegalArgumentException("k too large for 64-bit word");
  ...
  public static HBPIndexLong fromPackedWords(int kBits, int nCodes, long[] packedWords) {
    ...
    if (packedWords.length != expected) {
      throw new IllegalArgumentException(
          "packedWords.length mismatch: expected "
              + expected
              + " for k="
              + kBits
              + ", n="
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "lowKOnesRepeat: "
            + String.format("%64s", Long.toBinaryString(idx.lowKOnesRepeat)).replace(' ', '0'));
    System.out.println(
        "delimiterBitRepeat: "
            + String.format("%64s", Long.toBinaryString(idx.delimiterBitRepeat)).replace(' ', '0'));
    System.out.println(
        "addOneEachSection: "
            + String.format("%64s", Long.toBinaryString(idx.addOneEachSection)).replace(' ', '0'));
