...
echo ------------------------------------------
echo Starting Parquet to TsFile Script
echo ------------------------------------------
...
start /B /WAIT "" cmd /C "("%JAVA_HOME%\bin\java" -DTSFILE_HOME=!TSFILE_HOME! !tsfile_params! !JAVA_OPTS! -cp !CLASSPATH! !MAIN_CLASS! --format parquet %*)"
exit /b
